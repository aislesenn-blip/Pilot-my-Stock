-- UGAVISMART PRODUCTION REBUILD V3
-- Author: Jules (Lead Architect)
-- Objective: Full Schema Rebuild matching Frontend Logic 100%.
...
CREATE TABLE public.organizations (
    ...
    name TEXT NOT NULL,
    base_currency TEXT DEFAULT 'TZS', -- Required by app.js initCurrency
...
CREATE TABLE public.profiles (
    ...
    assigned_location_id UUID REFERENCES public.locations(id), -- Required for Strict Router
...
CREATE TABLE public.staff_invites (
    ...
    email TEXT NOT NULL,
    role TEXT NOT NULL,
    assigned_location_id UUID REFERENCES public.locations(id), -- Critical for "Ghost User" fix
...
CREATE TABLE public.products (
    ...
    category TEXT,
    unit TEXT,
    cost_price NUMERIC DEFAULT 0, -- Used for P&L Snapshot
    selling_price NUMERIC DEFAULT 0,
    currency TEXT DEFAULT 'TZS',
    conversion_factor NUMERIC DEFAULT 1, -- From addProductModal
...
CREATE TABLE public.purchase_orders (
    ...
    organization_id UUID REFERENCES public.organizations(id),
    supplier_name TEXT,
    supplier_id UUID, -- Added based on app.js `supplier_id`
...
CREATE TABLE public.change_requests (
    ...
    target_id UUID,
    action TEXT,
    new_data JSONB, -- Required for app.js stock edit
    status TEXT DEFAULT 'pending',
...
-- Profiles: Allow Self Access (Recursion Breaker) + Org View
CREATE POLICY "Profiles Self" ON public.profiles FOR ALL TO authenticated USING (auth.uid() = id) WITH CHECK (auth.uid() = id);
CREATE POLICY "Profiles Org" ON public.profiles FOR SELECT TO authenticated USING (organization_id = (SELECT organization_id FROM public.profiles WHERE id = auth.uid()));

-- Organizations: Linked Only
CREATE POLICY "Org View" ON public.organizations FOR SELECT TO authenticated USING (id = (SELECT organization_id FROM public.profiles WHERE id = auth.uid()));

-- All Other Tables: Standard Org Check
CREATE POLICY "Loc View" ON public.locations FOR ALL TO authenticated USING (organization_id = (SELECT organization_id FROM public.profiles WHERE id = auth.uid()));
...
CREATE POLICY "Rates View" ON public.exchange_rates FOR ALL TO authenticated USING (organization_id = (SELECT organization_id FROM public.profiles WHERE id = auth.uid()));
CREATE POLICY "Sup View" ON public.suppliers FOR ALL TO authenticated USING (organization_id = (SELECT organization_id FROM public.profiles WHERE id = auth.uid()));

-- 4. ATOMIC RPCs (SECURITY DEFINER = BYPASS RLS)
...
-- F. DEDUCT STOCK (Legacy/Single Support)
-- In case app.js calls it directly in legacy paths
...
SELECT 'Production Rebuild V3 Complete' as status;
