...
async function renderInventory(c) {
    try {
        ...
        c.innerHTML = `
            ...
            <div><h3 class="text-xs font-bold text-gray-400 uppercase mb-4 tracking-widest">2. Master Product Catalog</h3><div class="table-card"><div class="table-scroll"><table><thead><tr><th>Product Name</th><th>Standard Pricing</th><th class="text-right">Status</th></tr></thead><tbody>${catalogRows}</tbody></table></div></div></div>`;
    } catch(e) { console.error(e); }
}

// 2. REPORTS (Fixed Table)
...
async function renderStaff(c) {
    ...
    c.innerHTML = `<div class="flex justify-between items-center mb-8"><h1 class="text-2xl font-bold uppercase text-gray-900">Team Management</h1><button onclick="window.inviteModal()" class="btn-primary w-auto px-6 py-3 text-xs">+ Invite User</button></div><div class="table-card"><div class="table-scroll"><table><tbody>
...
async function renderApprovals(c) {
    ...
    c.innerHTML = `<h1 class="text-xl font-bold mb-8 uppercase text-gray-400">Approvals Center</h1><div class="table-card"><div class="table-scroll"><table><tbody>${rows.length?rows:'<tr><td colspan="4" class="text-center text-xs text-gray-400 py-8">No pending approvals.</td></tr>'}</tbody></table></div></div>`;
}
window.approve=async(id,s)=>{ if(confirm('Confirm?')) try { await respondToApproval(id,s,profile.id); router('approvals'); } catch(e){} }

// 6. SETTINGS (Fixed Table)
async function renderSettings(c) { 
    const { data: locs } = await supabase.from('locations').select('*').eq('organization_id', profile.organization_id); 
    c.innerHTML = `<div class="flex justify-between items-center mb-10"><h1 class="text-2xl font-bold uppercase text-gray-900">Settings</h1><button onclick="window.addStoreModal()" class="btn-primary w-auto px-6 py-3 text-xs">+ Add Hub</button></div><div class="table-card"><div class="table-scroll"><table><tbody>${locs.map(l=>`<tr><td class="font-bold text-sm uppercase">${l.name}</td><td class="text-green-600 font-bold text-[9px] text-right uppercase tracking-wider">ACTIVE</td></tr>`).join('')}</tbody></table></div></div>`; 
}

// MODALS (ILE ULIYOIPENDA: GAP & INPUT GROUP)
