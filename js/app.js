...
// --- REAL-TIME INVENTORY (WITH DATA SYNC) ---
            ...
            <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
                <table class="w-full text-left">
                    <thead class="bg-gray-50 text-[10px] uppercase text-gray-400 font-bold border-b">
                        <tr><th class="p-4">SKU / Item</th><th class="p-4">Store</th><th class="p-4">Balance</th><th class="p-4 text-right">Action</th></tr>
                    </thead>
                    <tbody>${rows.length ? rows : '<tr><td colspan="4" class="p-12 text-center text-xs text-gray-400 uppercase font-bold">No items found.</td></tr>'}</tbody>
                </table>
            </div>`;
    } catch(e) {}
}

// --- RECEIVE STOCK MODAL (DROPDOWN FIX) ---
    ...
    document.getElementById('modal-content').innerHTML = `
        ...
        <div class="space-y-5">
            <div class="input-group">
                ...
                <select id="sP" class="input-field" style="z-index: 100;">${prods.map(p=>`<option value="${p.id}">${p.name.toUpperCase()}</option>`).join('')}</select>
            </div>
            <div class="input-group">
                <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1 block">2. Destination Store</label>
            ...
            <button id="btnSaveStock" onclick="window.execAddStock()" class="btn-black w-full py-4 rounded-2xl font-bold text-[10px] uppercase tracking-widest mt-2 shadow-xl shadow-black/10">Authorize Stock Entry</button>
        </div>`;
...
// Register Product Modal (UX Ready)
window.addProductModal = () => {
    document.getElementById('modal-content').innerHTML = `
        ...
        <div class="space-y-4">
            ...
            <button id="btnSaveProd" onclick="window.execAddProduct()" class="btn-black w-full py-4 rounded-2xl font-bold text-[10px] uppercase mt-2">Add to Item Master</button>
        </div>`;
    document.getElementById('modal').classList.remove('hidden');
    document.getElementById('modal').classList.add('flex');
}
...
window.execAddProduct = async () => {
    ...
    try {
        ...
        document.getElementById('modal').classList.remove('flex');
        await router('inventory');
    } catch(e) { alert(e.message); btn.innerText = "Add to Item Master"; btn.disabled = false; }
}
...
window.renderCart=()=>{
    ...
    if(!cart.length){l.innerHTML='<p class="text-center py-10 text-gray-300 text-[10px] font-bold uppercase">Empty</p>'; t.innerText='$0.00'; return;}
    let sum=0; l.innerHTML=cart.map(i=>{sum+=i.price*i.qty; return `<div class="flex justify-between items-center text-xs font-bold uppercase"><span>${i.name} (${i.qty})</span><button onclick="window.remCart('${i.id}')" class="text-red-500">×</button></div>`}).join(''); t.innerText='$'+sum.toFixed(2);
}
window.remCart=(id)=>{cart=cart.filter(c=>c.id!==id); window.renderCart();}
window.checkout=async()=>{if(!cart.length) return; try{await processBarSale(profile.organization_id, profile.assigned_location_id, cart.map(c=>({product_id:c.id,qty:c.qty,price:c.price})), profile.id); alert('Sale Confirmed.'); cart=[]; window.renderCart(); await router('bar');}catch(e){}}

window.issueModal=(name,id,from)=>{
    document.getElementById('modal-content').innerHTML=`
        <h3 class="font-bold text-lg mb-6 uppercase tracking-tight">Issue Stock</h3>
        <div class="space-y-4">
            <input value="${name}" disabled class="input-field bg-gray-50 font-bold uppercase text-gray-400">
            <div class="input-group">
                <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1 block">Destination</label>
                <select id="tTo" class="input-field"></select>
            </div>
            <div class="input-group">
                <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1 block">Quantity</label>
                <input id="tQty" type="number" class="input-field" placeholder="0.00">
            </div>
            <button onclick="window.execIssue('${id}','${from}')" class="btn-black w-full py-4 rounded-2xl font-bold text-[10px] uppercase">Confirm Movement</button>
        </div>`;
...
window.execIssue=async(id,from)=>{ try { await transferStock(id, from, document.getElementById('tTo').value, document.getElementById('tQty').value, profile.id, profile.organization_id); document.getElementById('modal').classList.add('hidden'); document.getElementById('modal').classList.remove('flex'); await router('inventory'); } catch(e){alert(e.message);} }
...
async function renderApprovals(c){
    try {
        ...
        c.innerHTML = `<h1 class="text-xl font-bold mb-8 uppercase text-gray-400 font-bold">Control Queue</h1><div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm"><table class="w-full text-left"><tbody>${r.length?r:'<tr><td class="p-12 text-center text-xs text-gray-400 font-bold uppercase">All movements cleared.</td></tr>'}</tbody></table></div>`;
    } catch(e){}
}
...
async function renderSettings(c){
    ...
    const rows = locs.map(l => `<tr class="border-b"><td class="p-4 font-bold text-sm uppercase">${l.name}</td><td class="p-4 uppercase text-[10px] font-bold text-gray-400 tracking-widest">${l.type}</td><td class="p-4 text-right"><button onclick="window.editStoreModal('${l.id}','${l.name}','${l.type}')" class="text-blue-500 font-bold text-[10px] uppercase">Edit</button></td></tr>`).join('');
    c.innerHTML = `<div class="flex justify-between mb-8"><h1>Store Infrastructure</h1><button onclick="window.addStoreModal()" class="btn-black px-4 py-2.5 rounded-xl text-[10px] font-bold uppercase">+ New Store</button></div><div class="bg-white rounded-2xl border shadow-sm overflow-hidden"><table class="w-full text-left"><tbody>${rows}</tbody></table></div>`;
}

window.addStoreModal=()=>{
    document.getElementById('modal-content').innerHTML=`<h3 class="font-bold text-lg mb-6 uppercase">Register Store</h3><input id="nN" class="input-field mb-3" placeholder="NAME"><select id="nT" class="input-field mb-6"><option value="main_store">Main Store</option><option value="camp_store">Camp Store</option></select><button id="btnStore" onclick="window.execAddStore()" class="btn-black w-full py-4 rounded-2xl font-bold text-[10px] uppercase">Authorize Registry</button>`;
    document.getElementById('modal').classList.remove('hidden');
    document.getElementById('modal').classList.add('flex');
}
window.execAddStore=async()=>{ try{ await createLocation(profile.organization_id, document.getElementById('nN').value, document.getElementById('nT').value); document.getElementById('modal').classList.add('hidden'); await router('settings'); }catch(e){alert(e.message);} };
...
async function renderStaff(c){
    try {
        ...
        c.innerHTML = `<div class="flex justify-between mb-8"><h1 class="text-xl font-bold uppercase tracking-widest text-gray-400">Security & Access</h1><button onclick="window.inviteModal()" class="bg-black text-white px-4 py-2.5 rounded-xl text-[10px] font-bold uppercase">Grant Access</button></div><div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm"><table class="w-full text-left"><tbody>${rows}</tbody></table></div>`;
    } catch(e){}
}
window.inviteModal=async()=>{
    const locs = await getLocations(profile.organization_id);
    document.getElementById('modal-content').innerHTML=`<h3 class="font-bold text-lg mb-6 uppercase">Invite Personnel</h3><input id="iE" class="input-field mb-3" placeholder="EMAIL ADDRESS"><select id="iR" class="input-field mb-3"><option value="storekeeper">Storekeeper</option><option value="barman">Barman</option><option value="finance">Finance Specialist</option></select><select id="iL" class="input-field mb-6">${locs.map(l=>`<option value="${l.id}">${l.name.toUpperCase()}</option>`).join('')}</select><button onclick="window.execInvite()" class="btn-black w-full py-4 rounded-2xl font-bold text-[10px] uppercase">SEND ACCESS PORTAL</button>`;
    document.getElementById('modal').classList.remove('hidden');
    document.getElementById('modal').classList.add('flex');
}
window.execInvite=async()=>{ try { await supabase.from('staff_invites').insert({ email: document.getElementById('iE').value, role: document.getElementById('iR').value, organization_id: profile.organization_id, assigned_location_id: document.getElementById('iL').value, status: 'pending' }); document.getElementById('modal').classList.add('hidden'); await router('staff'); } catch(e){alert(e.message);} };
