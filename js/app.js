...
window.showNotification = (message, type = 'success') => {
    ...
    div.className = `fixed top-5 right-5 px-6 py-4 rounded-xl text-white font-bold shadow-2xl z-[11000] flex items-center gap-3 transition-all duration-300 transform translate-y-0 ${type === 'success' ? 'bg-[#0F172A]' : 'bg-red-600'}`;
...
window.onload = async () => {
    ...
    try {
        ...
        document.body.innerHTML = `<div class="p-10 text-center text-red-600 font-bold">System Error: ${e.message}</div>`;
    }
};
