...
window.execReceivePO = async function(poId) {
    ...
    window.premiumConfirm("Confirm Receipt", "Update stock levels?", "Yes", async () => {
        try {
            // CALL NEW RPC
            const { error } = await supabase.rpc('receive_stock_complete', {
                p_po_id: poId,
                p_items: items,
...
window.renderSettings = async (c) => { c.innerHTML = '<h1 class="text-3xl font-bold">Settings</h1><p>Restricted to Admins.</p>'; };
