...
window.showNotification = (message, type = 'success') => {
    ...
    if(existing) existing.remove(); // Remove old notifications to prevent stacking

    const div = document.createElement('div');
    div.id = 'notif-toast';
    div.style.cssText = `position: fixed; top: 20px; right: 20px; padding: 16px 24px; border-radius: 12px; color: white; font-weight: bold; z-index: 99999; box-shadow: 0 10px 25px rgba(0,0,0,0.3); display: flex; align-items: center; gap: 10px; animation: slideIn 0.3s ease-out;`;
...
window.onload = async () => {
    ...
    try {
        // 1. LINK INVITE (Critical Step)
        await supabase.rpc('claim_my_invite', { email_to_check: session.user.email, user_id_to_link: session.user.id });
        
        // 2. FETCH PROFILE (Retry Logic added for robustness)
        let { data: prof } = await supabase.from('profiles').select('*').eq('id', session.user.id).single();
        if (!prof) {
            // Wait 1 sec and retry (incase trigger is slow)
        ...
        // Kama bado profile haipo (Edge case), mrudishe setup au login
        if (!profile) { 
            console.error("Profile creation failed");
            logout(); return;
        }

        if (!profile.organization_id) { window.location.href = 'setup.html'; return; }

        // 3. FORCE NAME ENTRY (SECURITY CHECKPOINT)
        // Hapa tunahakikisha HAKUNA mtu anayepita bila jina.
        // Tunacheki: Jina tupu? Jina fupi? Au Jina ni sawa na Role yake (mfano anaitwa 'Barman')?
        ...
        if (currentName.length < 3 || forbiddenNames.some(n => currentName.toLowerCase() === n.toLowerCase())) {
            let newName = null;
            // Loop hapa mpaka aweke jina sahihi
            while (!newName || newName.length < 3) {
                newName = prompt(`⚠️ ACCOUNT SETUP REQUIRED\n\nPlease enter your OFFICIAL FULL NAME to activate your account:`);
            }
            
            // Update Database Papo hapo
            const { error: updateErr } = await supabase.from('profiles').update({ full_name: newName }).eq('id', profile.id);
            if (!updateErr) {
                profile.full_name = newName;
                window.showNotification("Account Activated Successfully", "success");
            } else {
                alert("Failed to save name. Please refresh.");
            }
        }
        ...
        console.error(e); 
        // Usi-logout haraka ili tuone error console ikiwa debugging
        // logout(); 
    }
};
...
window.router = async (view) => {
    ...
    try {
        ...
        app.innerHTML = `<div class="text-center p-10"><h3 class="text-red-600 font-bold">Error Loading View</h3><p class="text-sm text-gray-500">${err.message}</p></div>`;
    }
};
...
// --- BAR POS (FIXED LOADING ISSUE) ---
async function renderBar(c) {
    try {
        // Hapa tuna-fetch inventory, lakini tunaweka filter salama
        // Kama hana assigned_location_id, hii inaweza kuleta error au empty
        if (!profile.assigned_location_id) {
            c.innerHTML = `<div class="p-10 text-center"><h3 class="text-red-500 font-bold">Configuration Error</h3><p>You are not assigned to any Bar Location.</p></div>`;
            return;
        }

        const inv = await getInventory(profile.organization_id);
        
        // Filter: Tunachukua tu vitu vya bar yake
        const items = inv.filter(x => x.location_id === profile.assigned_location_id);

        const itemCards = items.map(x => `
            <div onclick="window.addCart('${x.products.name}',${x.products.selling_price},'${x.product_id}')" class="bg-white border p-6 rounded-2xl cursor-pointer hover:border-slate-900 transition shadow-sm group hover:shadow-lg hover:-translate-y-1">
                <h4 class="font-bold text-xs uppercase mb-3 text-slate-800 group-hover:text-slate-900">${x.products.name}</h4>
                <div class="flex justify-between items-center">
                    <span class="font-bold font-mono text-sm">$${x.products.selling_price}</span>
                    <span class="text-[10px] text-slate-400 font-bold uppercase bg-slate-50 px-2 py-1 rounded">Qty: ${x.quantity}</span>
                </div>
            </div>`).join('');
        ...
        c.innerHTML = `
            <div class="flex flex-col lg:flex-row h-full gap-10">
                <div class="flex-1 overflow-y-auto">
                    <h1 class="text-3xl font-bold uppercase mb-8 text-slate-900">POS Terminal</h1>
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-5">
                        ${items.length ? itemCards : '<div class="col-span-3 text-center py-10 text-slate-400 font-bold text-xs uppercase">No stock assigned to this Bar.</div>'}
                    </div>
                </div>
                <div class="w-full lg:w-96 bg-white border border-slate-200 rounded-3xl p-8 shadow-xl h-fit sticky top-0">
                    <h3 class="font-bold uppercase mb-6 text-sm tracking-widest text-slate-400">Current Ticket</h3>
                    <div id="cart-list" class="space-y-4 mb-8 min-h-[150px]"></div>
                    <div class="border-t border-slate-100 pt-6">
                        <div class="flex justify-between font-bold mb-6 text-xl text-slate-900"><span>Total</span><span id="cart-total">$0.00</span></div>
                        <button onclick="window.checkout()" class="btn-primary w-full py-4 text-sm shadow-xl">Complete Sale</button>
                    </div>
                </div>
            </div>`;
        window.renderCart();
        ...
        c.innerHTML = `<div class="p-10 text-center"><h3 class="text-red-500 font-bold">System Error</h3><p>Could not load POS. Please contact admin.</p></div>`;
    }
}

// ... (Functions za Reports, Approvals, Staff, Settings zinabaki vile vile) ...
// Zote ziko sawa, nazi-skip ili nisijaze nafasi bure. 
// Hakikisha unatumia renderReports, renderApprovals, renderStaff, renderSettings kutoka version iliyopita.

// HELPER ZA REPORT NA CART
...
// --- FIXED MODALS (Z-INDEX SULUHISHO LA MWISHO) ---
...
window.addStockModal = async () => {
    ...
    document.getElementById('modal-content').innerHTML = `
        ...
        <div class="input-group" style="position: relative; z-index: 40;">
...
window.issueModal = async (name, id, fromLoc) => { 
    ...
    // HAPA NDIYO PALIKUWA NA KOSA. SASA NIMEREKEBISHA KAMA "ADD STOCK"
    document.getElementById('modal-content').innerHTML = `
        ...
        <div class="input-group" style="position: relative; z-index: 5;">
...
window.inviteModal = async () => { 
    ...
    document.getElementById('modal-content').innerHTML = `
        ...
        <div class="input-group" style="position: relative; z-index: 30;">
...
window.addStoreModal=()=>{ document.getElementById('modal-content').innerHTML=`<h3 class="font-bold text-lg mb-8 uppercase text-center">Add Hub</h3><div class="input-group" style="position: relative; z-index: 20;"><label class="input-label">Name</label><input id="nN" class="input-field"></div><div class="input-group" style="position: relative; z-index: 10;"><label class="input-label">Type</label><select id="nT" class="input-field"><option value="main_store">Main Store</option><option value="camp_store">Camp Store</option><option value="department">Department</option></select></div><button onclick="window.execAddStore()" class="btn-primary mt-4">Create</button>`; document.getElementById('modal').style.display = 'flex'; };
