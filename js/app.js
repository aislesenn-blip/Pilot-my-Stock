...
// --- 1. SYSTEM INITIALIZATION & SECURITY CHECK ---
window.onload = async () => {
...
// --- 2. PERMISSION LOGIC (WHO SEES WHAT) ---
function applyStrictPermissions(role) {
    ...
    // STRICT RULES AS PER PROPOSAL:
    if (role === 'manager') {
        // PROPOSAL PT 1: Central Control. Sees Everything.
        // Optional: Can hide POS if Manager doesn't sell. But we leave it open.
    } 
    else if (role === 'finance') {
        // PROPOSAL PT 3 & 5: Approval Controls & Reporting.
        // RESTRICTION: No Bar (Operations), No Settings (Config), No Staff (HR).
        hide(['nav-bar', 'nav-settings', 'nav-staff']); 
    } 
    else if (role === 'storekeeper') {
        // PROPOSAL PT 2: Camp Management.
        // RESTRICTION: No Approvals (Can't self-approve), No Bar, No Staff, No Settings.
        hide(['nav-bar', 'nav-approvals', 'nav-staff', 'nav-settings']);
    } 
    else if (role === 'barman') {
        // PROPOSAL PT 4: Dedicated Bar Module.
        // RESTRICTION: POS Only. No Inventory, No Reports.
        hide(['nav-inventory', 'nav-approvals', 'nav-reports', 'nav-staff', 'nav-settings']);
    }
}

// --- 3. ROUTER (PREVENTS UNAUTHORIZED ACCESS) ---
window.router = async (view) => {
    // SECURITY GATE: Even if they hack the UI, this blocks them.
    ...
    if (profile.role === 'finance' && ['bar', 'settings'].includes(view)) return alert("Access Denied.");
    ...
    try {
        ...
        c.innerHTML = `
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
                <div>
                    <h1 class="text-3xl font-bold uppercase tracking-tight text-slate-900">Inventory Control</h1>
                    <p class="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">${profile.role === 'manager' ? 'Main Store Overview' : 'Camp Store Balance'}</p>
                </div>
                <div class="flex gap-3">
                    ${showAdminActions ? `
                        <button onclick="window.addProductModal()" class="btn-primary bg-white text-slate-900 border border-slate-200 hover:bg-slate-50 shadow-sm">Register Item</button>
                        <button onclick="window.addStockModal()" class="btn-primary shadow-lg shadow-slate-900/20">Receive Stock</button>
                    ` : ''}
                </div>
            </div>
...
// REPORTS (Proposal Pt 5)
async function renderReports(c) {
    try {
        const { data: logs } = await supabase.from('transactions').select('*, products(name), locations:to_location_id(name), from_loc:from_location_id(name), profiles:user_id(full_name)').eq('organization_id', profile.organization_id).order('created_at', { ascending: false }).limit(50);
        
        // FILTER: Storekeeper sees logs related to THEIR camp only.
        const visibleLogs = (profile.role === 'manager' || profile.role === 'finance') 
            ? logs 
            : logs.filter(l => l.from_location_id === profile.assigned_location_id || l.to_location_id === profile.assigned_location_id);

        // PRIVACY: Only Manager/Finance see Money ($). Storekeeper sees Qty only.
        const showFinancials = (profile.role === 'manager' || profile.role === 'finance');
        const totalSales = visibleLogs.filter(l => l.type === 'sale').reduce((sum, l) => sum + (Number(l.total_value) || 0), 0);
        const totalProfit = visibleLogs.filter(l => l.type === 'sale').reduce((sum, l) => sum + (Number(l.profit) || 0), 0);

        const rows = visibleLogs.map(l => {
            let action = l.type.replace('_', ' ').toUpperCase();
            let color = 'text-blue-600';
            if(l.type === 'sale') color = 'text-green-600';
            // Proposal Pt 2: "Departmental Consumption"
            if(l.type === 'pending_transfer') { action = 'REQUESTED'; color = 'text-orange-500'; }
            if(l.type === 'transfer_completed' && l.locations?.type === 'department') { action = 'CONSUMPTION'; color = 'text-purple-600'; }

            return `<tr>
                <td class="text-gray-500 text-xs">${new Date(l.created_at).toLocaleDateString()}</td>
                <td class="font-bold uppercase">${l.products?.name}</td>
                <td class="font-bold uppercase ${color} text-[10px] tracking-wider">${action}</td>
                <td class="text-xs uppercase text-gray-400">${l.from_loc?.name || '-'} <span class="text-slate-300">➜</span> ${l.locations?.name || '-'}</td>
                <td class="font-mono font-bold">${l.quantity}</td>
                <td class="text-gray-500 uppercase text-[9px] font-bold tracking-wide">${l.profiles?.full_name || 'System'}</td>
            </tr>`;
        }).join('');
        ...
        c.innerHTML = `
            <div class="flex justify-between items-center mb-10">
                ...
                ${showFinancials ? '<button class="text-xs font-bold bg-slate-100 px-4 py-2 rounded uppercase">Export CSV</button>' : ''}
            </div>
            ${showFinancials ? `
            ...
            <h3 class="text-xs font-bold text-slate-400 uppercase mb-4 tracking-widest">Audit Trail</h3>
            <div class="table-card"><div class="table-wrapper"><table><thead><tr><th>Date</th><th>Item</th><th>Action</th><th>Route</th><th>Qty</th><th>User</th></tr></thead><tbody>${rows || '<tr><td colspan="6" class="text-center">No records.</td></tr>'}</tbody></table></div></div>`;
    } catch(e) { c.innerHTML = '<p class="text-red-500">Error.</p>'; }
}

// APPROVALS (Proposal Pt 3)
async function renderApprovals(c) {
    if(profile.role === 'storekeeper') return; // Double Check
    ...
    c.innerHTML = `<h1 class="text-3xl font-bold mb-10 uppercase text-slate-900">Approvals</h1><div class="table-card"><div class="table-wrapper"><table><tbody>${rows.length?rows:'<tr><td colspan="4" class="text-center text-xs text-slate-400 py-12">No pending approvals.</td></tr>'}</tbody></table></div></div>`;
}
...
async function renderStaff(c) {
    ...
    c.innerHTML = `<div class="flex justify-between items-center mb-10"><h1 class="text-3xl font-bold uppercase text-slate-900">Team</h1><button onclick="window.inviteModal()" class="btn-primary w-auto px-6 py-3 text-xs">+ Invite</button></div><div class="table-card"><div class="table-wrapper"><table><tbody>${active.map(s=>`<tr><td class="font-bold uppercase">${s.full_name}</td><td class="text-xs font-bold text-blue-600 uppercase">${s.role}</td><td class="text-right text-green-500 font-bold text-[10px] uppercase">ACTIVE</td></tr>`).join('')}${pending.map(i=>`<tr class="bg-yellow-50"><td class="text-sm font-medium text-slate-600">${i.email}</td><td class="text-xs font-bold text-slate-400 uppercase">${i.role}</td><td class="text-right text-yellow-600 font-bold text-[10px] uppercase">PENDING</td></tr>`).join('')}</tbody></table></div></div>`;
}
async function renderSettings(c) { 
    const { data: locs } = await supabase.from('locations').select('*').eq('organization_id', profile.organization_id); 
    c.innerHTML = `<div class="flex justify-between items-center mb-10"><h1 class="text-3xl font-bold uppercase text-slate-900">Locations</h1><button onclick="window.addStoreModal()" class="btn-primary w-auto px-6 py-3 text-xs">+ Add Hub</button></div><div class="table-card"><div class="table-wrapper"><table><tbody>${locs.map(l=>`<tr><td class="font-bold text-sm uppercase">${l.name}</td><td class="text-xs font-bold uppercase text-gray-400">${l.type.replace('_', ' ')}</td><td class="text-green-600 font-bold text-[9px] text-right uppercase bg-green-50 px-3 py-1 rounded-full inline-block mt-1">ACTIVE</td></tr>`).join('')}</tbody></table></div></div>`; 
}

// BAR POS (Proposal Pt 4)
...
window.addProductModal = () => { 
    ...
    document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-8 uppercase text-center">New Product</h3><div class="input-group"><label class="input-label">Name</label><input id="pN" class="input-field uppercase"></div><div class="grid grid-cols-2 gap-5 mb-8"><div class="input-group mb-0"><label class="input-label">Cost</label><input id="pC" type="number" class="input-field"></div><div class="input-group mb-0"><label class="input-label">Selling</label><input id="pS" type="number" class="input-field"></div></div><button onclick="window.execAddProduct()" class="btn-primary">Save</button>`;
...
window.issueModal = async (name, id, fromLoc) => { 
    ...
    // PROPOSAL PT 2 STRICT RULE: 
    // Storekeepers can only issue to DEPARTMENTS (Kitchen, etc). Not other Camps.
    if(profile.role === 'storekeeper') {
        locs = locs.filter(l => l.type === 'department');
    }

    document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-8 uppercase text-center">Move Stock</h3><div class="input-group"><label class="input-label">Item</label><input value="${name}" disabled class="input-field bg-slate-50 uppercase"></div><div class="input-group"><label class="input-label">To Destination</label><select id="tTo" class="input-field">${locs.map(l=>`<option value="${l.id}">${l.name} (${l.type.replace('_',' ')})</option>`).join('')}</select></div><div class="input-group"><label class="input-label">Qty</label><input id="tQty" type="number" class="input-field"></div><button onclick="window.execIssue('${id}','${fromLoc}')" class="btn-primary">Request Transfer</button>`;
    document.getElementById('modal').style.display = 'flex';
};

window.inviteModal = async () => { 
    const { data: locs } = await supabase.from('locations').select('*').eq('organization_id', profile.organization_id); 
    document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-8 uppercase text-center">Invite Staff</h3><div class="input-group"><label class="input-label">Email</label><input id="iE" class="input-field"></div><div class="input-group"><label class="input-label">Role</label><select id="iR" class="input-field"><option value="storekeeper">Storekeeper</option><option value="barman">Barman</option><option value="finance">Finance</option></select></div><div class="input-group"><label class="input-label">Assign Location</label><select id="iL" class="input-field">${locs.map(l=>`<option value="${l.id}">${l.name}</option>`).join('')}</select></div><button onclick="window.execInvite()" class="btn-primary">Send</button>`; 
    document.getElementById('modal').style.display = 'flex'; 
};
