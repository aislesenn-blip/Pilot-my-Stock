...
window.onload = async () => {
    // 1. Loading Screen (Kuzuia UI kuwaka na kuzima)
    const app = document.getElementById('app-view');
    if(app) app.innerHTML = '<div class="flex h-screen items-center justify-center flex-col"><div class="w-8 h-8 border-2 border-black border-t-transparent rounded-full animate-spin mb-4"></div><p class="text-xs font-bold text-gray-500 tracking-widest">LOADING SYSTEM...</p></div>';

    const session = await getSession();
    if (!session) { window.location.href = 'index.html'; return; }

    try {
        // 2. FORCE CHECK: Angalia Invite KWANZA kabla ya Profile
        // Hii inazuia ile "Race Condition" ambapo user mpya anapelekwa Setup kimakosa
        ...
        if (invite) {
            ...
            // Tengeneza au Update Profile kwa kutumia Data za Invite
            const { data: forcedProfile, error } = await supabase.from('profiles').upsert({
                id: session.user.id,
                email: session.user.email,
                full_name: 'Staff Member', // Ata-update baadaye
                organization_id: invite.organization_id,
                role: invite.role,
                assigned_location_id: invite.assigned_location_id
            }).select().single();

            if (error) { console.error(error); logout(); return; }
            ...
            // Kama hana invite, jaribu kuvuta profile ya kawaida
            profile = await getCurrentProfile(session.user.id);
            
            // Kama Profile haipo au haina Organization -> Nenda Setup (Manager Mpya)
            if (!profile || !profile.organization_id) {
                console.log("No invite & No Profile. Redirecting to Setup.");
...
// ... (Router na Functions zingine zinabaki kama zilivyo) ...
...
// Functions za UI (Copy zilezile za zamani)
...
// --- HII HAPA NDIO FIX YA INVITE EMAIL ---
// Nimeongeza ukaguzi (Validation) kabla ya kutuma.
window.inviteModal=()=>{
...
window.sI = async () => {
    ...
    // VALIDATION: Hakikisha email ipo
    ...
    try {
        await inviteStaff(emailField.value, roleField.value, profile.organization_id); 
