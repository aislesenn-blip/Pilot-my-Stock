...
async function renderInventory(c) {
    try {
        ...
        c.innerHTML = `
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10"><h1 class="text-3xl font-bold uppercase tracking-tight text-slate-900">Stock Dashboard</h1><div class="flex gap-3 w-full md:w-auto">${profile.role === 'manager' ? `<button onclick="window.addProductModal()" class="btn-primary bg-white text-slate-900 border border-slate-200 hover:bg-slate-50 flex-1 md:flex-none shadow-sm">Register Item</button>` : ''}<button onclick="window.addStockModal()" class="btn-primary flex-1 md:flex-none shadow-lg shadow-slate-900/20">Receive Stock</button></div></div>
            <div class="mb-12"><h3 class="text-xs font-bold text-slate-400 uppercase mb-4 tracking-widest">1. Physical Stock Balance</h3><div class="table-card"><div class="table-scroll"><table><thead><tr><th>Item</th><th>Location</th><th>Qty</th><th class="text-right">Action</th></tr></thead><tbody>${stockRows.length ? stockRows : '<tr><td colspan="4" class="text-center text-xs text-gray-400 py-12">No stock available in this location.</td></tr>'}</tbody></table></div></div></div>
            <div><h3 class="text-xs font-bold text-slate-400 uppercase mb-4 tracking-widest">2. Master Product Catalog</h3><div class="table-card"><div class="table-scroll"><table><thead><tr><th>Product Name</th><th>Standard Pricing</th><th class="text-right">Status</th></tr></thead><tbody>${catalogRows}</tbody></table></div></div></div>`;
    } catch(e) { console.error(e); }
}

// 2. REPORTS (Premium Cards)
...
async function renderApprovals(c) {
    ...
    c.innerHTML = `<h1 class="text-3xl font-bold mb-10 uppercase text-slate-900">Approvals Center</h1><div class="table-card"><div class="table-scroll"><table><tbody>${rows.length?rows:'<tr><td colspan="4" class="text-center text-xs text-slate-400 py-12">No pending approvals.</td></tr>'}</tbody></table></div></div>`;
}
...
async function renderSettings(c) { 
    ...
    c.innerHTML = `<div class="flex justify-between items-center mb-10"><h1 class="text-3xl font-bold uppercase text-slate-900">System Config</h1><button onclick="window.addStoreModal()" class="btn-primary w-auto px-8 py-3 text-xs shadow-lg shadow-slate-900/20">+ Add Hub</button></div><div class="table-card"><div class="table-scroll"><table><tbody>${locs.map(l=>`<tr><td class="font-bold text-sm uppercase">${l.name}</td><td class="text-green-600 font-bold text-[9px] text-right uppercase tracking-wider bg-green-50 px-3 py-1 rounded-full inline-block mt-1">ACTIVE</td></tr>`).join('')}</tbody></table></div></div>`; 
}

// MODALS (FIXED SPACING)
window.addProductModal = () => { 
    document.getElementById('modal-content').innerHTML = `
        ...
        <button onclick="window.execAddProduct()" class="btn-primary mt-6">Save Item</button>`;
...
window.issueModal = async (name, id, fromLoc) => { 
    ...
    document.getElementById('modal-content').innerHTML = `
        ...
        <div class="input-group"><label class="input-label">Product</label><input value="${name}" disabled class="input-field bg-slate-50 font-bold uppercase text-slate-500 input-disabled"></div>
...
window.addStoreModal=()=>{ document.getElementById('modal-content').innerHTML=`<h3 class="font-bold text-lg mb-8 uppercase tracking-tight text-center text-slate-800">Add Hub</h3><div class="input-group"><label class="input-label">Hub Name</label><input id="nN" class="input-field" placeholder="e.g. Kitchen"></div><div class="input-group"><label class="input-label">Hub Type</label><select id="nT" class="input-field"><option value="main_store">Main Store</option><option value="camp_store">Camp Store</option><option value="department">Department</option></select></div><button onclick="window.execAddStore()" class="btn-primary mt-6">Create</button>`; document.getElementById('modal').style.display = 'flex'; };
