...
async function renderInventory(c) {
    try {
        ...
        const stockRows = filteredStock.map(i => `<tr class="border-b hover:bg-gray-50"><td class="p-4 font-bold text-sm uppercase">${i.products?.name}</td><td class="p-4 text-xs uppercase font-bold tracking-widest text-gray-500">${i.locations?.name}</td><td class="p-4 font-mono font-bold">${i.quantity}</td><td class="p-4 text-right">${profile.role !== 'barman' ? `<button onclick="window.issueModal('${i.products.name}','${i.product_id}','${i.location_id}')" class="btn-primary py-2 px-3">MOVE</button>` : ''}</td></tr>`).join('');
        ...
        c.innerHTML = `
            <div class="flex justify-between items-center mb-8"><h1 class="text-2xl font-bold uppercase tracking-tight">Stock Dashboard</h1><div class="flex gap-2"><button onclick="window.addProductModal()" class="btn-primary bg-white border border-gray-300 text-black hover:bg-gray-50">Register Item</button><button onclick="window.addStockModal()" class="btn-primary">Receive Stock</button></div></div>
            <div class="mb-10"><h3 class="text-[10px] font-bold text-gray-400 uppercase mb-3 tracking-widest">1. Stock Levels</h3><div class="table-responsive"><table class="w-full text-left min-w-[600px]"><thead class="bg-gray-50 text-[10px] uppercase font-bold text-gray-400 border-b"><tr><th class="p-4">Item</th><th class="p-4">Store</th><th class="p-4">Qty</th><th class="p-4 text-right">Action</th></tr></thead><tbody>${stockRows.length ? stockRows : '<tr><td colspan="4" class="p-8 text-center text-xs text-gray-400">No stock.</td></tr>'}</tbody></table></div></div>
            <div><h3 class="text-[10px] font-bold text-gray-400 uppercase mb-3 tracking-widest">2. Master Catalog</h3><div class="table-responsive"><table class="w-full text-left min-w-[600px]"><thead class="bg-gray-50 text-[10px] uppercase font-bold text-gray-400 border-b"><tr><th class="p-4">Item</th><th class="p-4">Pricing</th><th class="p-4 text-right">Status</th></tr></thead><tbody>${catalogRows}</tbody></table></div></div>`;
    } catch(e) { console.error(e); }
}

// 2. REPORTS (FIXED LOADING)
async function renderReports(c) {
    try {
        ...
        const rows = (logs && logs.length) ? logs.map(l => `<tr class="border-b text-[11px]"><td class="p-4 text-gray-500">${new Date(l.created_at).toLocaleDateString()}</td><td class="p-4 font-bold uppercase">${l.products?.name || 'N/A'}</td><td class="p-4 font-bold uppercase ${l.type.includes('sale') ? 'text-green-600' : 'text-blue-600'}">${l.type.replace('_', ' ')}</td><td class="p-4 font-mono">${l.quantity}</td><td class="p-4 text-gray-400 uppercase">${l.profiles?.full_name || 'System'}</td></tr>`).join('') : '';
        ...
        c.innerHTML = `
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                <div class="bg-white p-6 border border-gray-200 rounded-xl shadow-sm">
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">Total Sales Revenue</p>
                    <p class="text-3xl font-bold font-mono text-gray-900">$${totalSales.toLocaleString()}</p>
                </div>
                <div class="bg-white p-6 border border-gray-200 rounded-xl shadow-sm">
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">Total Gross Profit</p>
                    <p class="text-3xl font-bold font-mono text-green-600">$${totalProfit.toLocaleString()}</p>
                </div>
            </div>
            <h3 class="text-[11px] font-bold text-gray-400 uppercase mb-3 tracking-widest">Transaction Audit Trail</h3>
            <div class="table-responsive shadow-sm"><table class="w-full text-left min-w-[600px]"><thead class="bg-gray-50 text-[10px] uppercase font-bold text-gray-500 border-b"><tr><th class="p-4">Date</th><th class="p-4">Item</th><th class="p-4">Action</th><th class="p-4">Qty</th><th class="p-4">User</th></tr></thead><tbody>${rows || '<tr><td colspan="5" class="p-8 text-center text-xs text-gray-400">No transactions found.</td></tr>'}</tbody></table></div>`;
...
async function renderStaff(c) {
    ...
    c.innerHTML = `<div class="flex justify-between mb-8"><h1 class="text-2xl font-bold uppercase">Team</h1><button onclick="window.inviteModal()" class="btn-primary">+ Invite</button></div>
    <div class="table-responsive shadow-sm"><table class="w-full text-left min-w-[600px]"><tbody>
    ${active.map(s=>`<tr class="border-b"><td class="p-4 text-sm font-bold uppercase">${s.full_name}</td><td class="p-4 text-[10px] font-bold text-blue-600 uppercase">${s.role}</td><td class="p-4 text-right text-green-500 font-bold text-[9px] uppercase">ACTIVE</td></tr>`).join('')}
...
async function renderApprovals(c) {
    ...
    const rows = q.map(x => `<tr class="border-b"><td class="p-4 font-bold text-sm uppercase">${x.products?.name}</td><td class="p-4 font-bold text-blue-600">${x.quantity}</td><td class="p-4 text-xs text-gray-500">To: ${x.to_loc?.name}</td><td class="p-4 text-right"><button onclick="window.approve('${x.id}','approved')" class="btn-primary py-2 px-4">Approve</button></td></tr>`).join('');
    c.innerHTML = `<h1 class="text-xl font-bold mb-8 uppercase text-gray-400">Approvals</h1><div class="table-responsive shadow-sm"><table class="w-full text-left"><tbody>${rows.length?rows:'<tr><td class="p-12 text-center text-xs text-gray-400 uppercase font-bold">Queue clear.</td></tr>'}</tbody></table></div>`;
}
window.approve=async(id,s)=>{ if(confirm('Approve?')) try { await respondToApproval(id,s,profile.id); router('approvals'); } catch(e){} }

// MODALS (UI Fixed - Rounded & Padding)
window.issueModal = async (name, id, fromLoc) => {
    ...
    document.getElementById('modal-content').innerHTML = `
        <h3 class="font-bold text-lg mb-6 uppercase tracking-tight">Move Stock</h3>
        <div class="modal-body">
            <input value="${name}" disabled class="input-field bg-gray-50 font-bold uppercase">
            <div class="z-high"><label class="text-[10px] font-bold uppercase block mb-1">To (Camp/Dept)</label><select id="tTo" class="input-field">${locs.map(l=>`<option value="${l.id}">${l.name}</option>`).join('')}</select></div>
            <div><label class="text-[10px] font-bold uppercase block mb-1">Qty</label><input id="tQty" type="number" class="input-field"></div>
            <button onclick="window.execIssue('${id}','${fromLoc}')" class="btn-primary w-full py-3 mt-2">Request Transfer</button>
        </div>`;
...
window.addProductModal = () => { document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-6 uppercase">Register Item</h3><div class="modal-body"><input id="pN" class="input-field uppercase" placeholder="NAME"><div class="flex gap-4"><input id="pC" type="number" class="input-field" placeholder="COST"><input id="pS" type="number" class="input-field" placeholder="SELL"></div><button onclick="window.execAddProduct()" class="btn-primary w-full py-3">Save</button></div>`; document.getElementById('modal').style.display = 'flex'; };
...
async function renderSettings(c) { const { data: locs } = await supabase.from('locations').select('*').eq('organization_id', profile.organization_id); c.innerHTML = `<div class="flex justify-between items-center mb-10"><h1 class="text-2xl font-bold uppercase">Settings</h1><button onclick="window.addStoreModal()" class="btn-primary">+ Hub</button></div><div class="table-responsive shadow-sm"><table class="w-full text-left min-w-[600px]"><tbody>${locs.map(l=>`<tr class="border-b"><td class="p-4 font-bold text-sm uppercase">${l.name}</td><td class="p-4 text-green-500 font-bold text-[9px] text-right uppercase">ACTIVE</td></tr>`).join('')}</tbody></table></div>`; }
window.addStoreModal=()=>{ document.getElementById('modal-content').innerHTML=`<h3 class="font-bold text-lg mb-6 uppercase">Add Hub</h3><div class="modal-body"><input id="nN" class="input-field" placeholder="NAME"><select id="nT" class="input-field"><option value="main_store">Main Store</option><option value="camp_store">Camp Store</option><option value="department">Department</option></select><button onclick="window.execAddStore()" class="btn-primary w-full py-3">Create</button></div>`; document.getElementById('modal').style.display = 'flex'; };
