...
window.onload = async () => {
    ...
    if(app) app.innerHTML = '<div class="flex h-screen items-center justify-center flex-col"><div class="w-8 h-8 border-2 border-black border-t-transparent rounded-full animate-spin mb-4"></div><p class="text-xs font-bold text-gray-400 tracking-widest">LOADING SYSTEM...</p></div>';
    ...
    try {
        ...
        // 2. CHECK: KAMA HANA ORGANIZATION (Setup Loop Prevention)
        if (!profile || !profile.organization_id) {
            
            // JARIBU KUTAFUTA INVITE KWA MKONO (Backup Plan)
            const { data: invite } = await supabase.from('staff_invites').select('*').eq('email', session.user.email).maybeSingle();
            
            if (invite) {
                // KAMA INVITE IPO: Mwingize kwa nguvu (Force Entry)
                ...
                // KAMA HANA INVITE: Nenda Setup (Manager)
...
// --- INVITE STAFF MODAL (FIXED VALIDATION) ---
window.inviteModal=()=>{
    document.getElementById('modal-content').innerHTML=`
    <h3 class="font-bold mb-2">Invite</h3>
    <form onsubmit="event.preventDefault();window.sI()">
        <input id="iE" class="input-field w-full mb-2" placeholder="Email" type="email">
        ...
        <button class="btn-black w-full py-2 rounded">Send</button>
    </form>`;
...
window.sI = async () => {
    ...
    // VALIDATION: Hii inazuia "Invite Sent" kama email ni tupu
    if (!email || !email.trim()) {
