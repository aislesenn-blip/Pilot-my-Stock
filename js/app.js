...
// 🔥 LIST KAMILI YA PESA ZOTE (HAKUNA "MANY MORE")
const ALL_CURRENCIES = [
    'TZS', 'USD', 'EUR', 'GBP', 'KES', 'UGX', 'RWF', 'ZAR', 'AED', 'CNY', 
    'INR', 'CAD', 'AUD', 'JPY', 'CHF', 'SAR', 'QAR', 'BWP', 'NGN', 'GHS'
];
...
// 🔥 SMART CURRENCY FORMATTER (With Alert)
window.formatPrice = (amount) => {
    if (!amount && amount !== 0) return '-';
    let converted = window.convertAmount(amount, window.baseCurrency, window.selectedCurrency);
    
    // Kama rate haipo, onyesha SET RATE button
    if (converted === null) {
        return `<button onclick="window.router('settings')" class="text-[9px] font-bold bg-red-50 text-red-600 border border-red-200 px-2 py-1 rounded hover:bg-red-100 cursor-pointer">SET RATE</button>`;
    }
...
window.renderReports = async (c) => {
    ...
    if(isVariance) {
        ...
        c.innerHTML = `<div class="flex flex-col gap-6 mb-8"><div class="flex justify-between items-center gap-4"><div class="flex items-center gap-4"><h1 class="text-3xl font-bold uppercase text-slate-900 tracking-tight">Reports</h1>${window.getCurrencySelectorHTML()}</div><div class="flex gap-1 bg-slate-100 p-1 rounded-lg"><button class="px-6 py-2 text-xs font-bold rounded-md bg-white shadow-sm text-slate-900 transition">GENERAL</button><button onclick="window.currentRepView='variance'; window.router('reports')" class="px-6 py-2 text-xs font-bold rounded-md text-slate-500 hover:text-slate-900 transition">VARIANCE</button></div></div><div class="bg-white p-4 rounded-2xl border border-slate-200 flex flex-wrap items-end gap-4 shadow-sm"><div class="flex-1"><label class="input-label">Start Date</label><input type="date" id="repStart" class="input-field" onchange="window.filterReport()"></div><div class="flex-1"><label class="input-label">End Date</label><input type="date" id="repEnd" class="input-field" onchange="window.filterReport()"></div><div class="flex-1"><label class="input-label">Location</label><select id="repLoc" class="input-field" onchange="window.filterReport()"><option value="all">Consolidated (All Camps)</option>${window.cachedLocations.map(l=>`<option value="${l.name}">${l.name}</option>`).join('')}</select></div><button onclick="window.exportCSV()" class="btn-primary w-auto px-6 h-[46px] bg-green-700 hover:bg-green-800">EXPORT</button></div></div><div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8"><div class="bg-white p-8 border border-slate-200 rounded-[24px] shadow-sm"><p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Total Revenue</p><p class="text-4xl font-mono font-bold text-slate-900" id="repRev">...</p></div><div class="bg-white p-8 border border-slate-200 rounded-[24px] shadow-sm"><p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Gross Profit</p><p class="text-4xl font-mono font-bold text-green-600" id="repProf">...</p></div></div><div class="bg-white rounded-[24px] border border-slate-200 shadow-sm overflow-hidden"><div class="overflow-x-auto"><table class="w-full text-left"><thead class="bg-slate-50 border-b border-slate-100"><tr><th class="p-4 pl-6 text-xs text-slate-400 uppercase tracking-widest">Date / Time</th><th class="text-xs text-slate-400 uppercase tracking-widest">User Profile</th><th class="text-xs text-slate-400 uppercase tracking-widest">Item Detail</th><th class="text-xs text-slate-400 uppercase tracking-widest">Transaction</th><th class="text-xs text-slate-400 uppercase tracking-widest">Qty</th></tr></thead><tbody id="logsBody"></tbody></table></div></div>`;
        window.filterReport();
    }
};
window.filterReport = () => { const start=document.getElementById('repStart')?.value, end=document.getElementById('repEnd')?.value, loc=document.getElementById('repLoc')?.value; let f=window.currentLogs; if(start) f=f.filter(l=>new Date(l.created_at)>=new Date(start)); if(end) f=f.filter(l=>new Date(l.created_at)<=new Date(end+'T23:59:59')); if(loc&&loc!=='all') f=f.filter(l=>(l.locations?.name===loc||l.from_loc?.name===loc)); const totalSales=f.filter(l=>l.type==='sale').reduce((sum,l)=>sum+(Number(l.total_value)||0),0), totalProfit=f.filter(l=>l.type==='sale').reduce((sum,l)=>sum+(Number(l.profit)||0),0); document.getElementById('repRev').innerText=window.formatPrice(totalSales); document.getElementById('repProf').innerText=window.formatPrice(totalProfit); const b=document.getElementById('logsBody'); if(!b) return; b.innerHTML=f.map(l=>{ const locName=window.cachedLocations.find(loc=>loc.id===l.profiles?.assigned_location_id)?.name||'Unassigned'; let tag=l.type.toUpperCase(), det=`${l.from_loc?.name||'-'} ➜ ${l.locations?.name||'-'}`, badge='bg-slate-100 text-slate-500'; if(l.type==='sale'){tag='SALE'; det='POS Transaction'; badge='bg-green-50 text-green-700 border-green-100';} if(l.type==='receive'){tag='STOCK IN'; det='Supplier Entry'; badge='bg-blue-50 text-blue-700 border-blue-100';} return `<tr class="border-b border-slate-50 hover:bg-slate-50 transition group"><td class="p-4 pl-6 text-xs font-bold text-slate-500">${new Date(l.created_at).toLocaleDateString()} <span class="text-[10px] font-normal opacity-50 block">${new Date(l.created_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</span></td><td class="p-4"><button onclick="window.viewUserProfile('${l.profiles?.id}')" class="text-left group-hover:translate-x-1 transition-transform"><div class="font-bold text-xs uppercase text-slate-900">${l.profiles?.full_name||'System'}</div><div class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">${l.profiles?.role||'-'} &bull; ${locName}</div></button></td><td class="p-4"><div class="font-bold text-xs uppercase text-slate-700">${l.products?.name||'Unknown'}</div><div class="text-[9px] text-slate-400 uppercase mt-0.5">${l.products?.category||'-'}</div></td><td class="p-4"><span class="text-[9px] font-bold px-2 py-1 rounded border ${badge}">${tag}</span><div class="text-[10px] font-bold text-slate-400 uppercase mt-1 tracking-wide">${det}</div></td><td class="p-4 font-mono text-sm font-bold text-slate-900">${l.quantity}</td></tr>`;}).join(''); };
...
window.renderStaff = async (c) => {
    ...
    ${staff.map(s => {
        ...
        return `<tr class="border-b border-slate-50 hover:bg-slate-50 transition"><td class="p-6"><div class="font-bold text-sm uppercase text-slate-700">${s.full_name}</div><div class="text-[10px] uppercase font-bold text-blue-600 mt-1">${s.role}</div></td><td class="text-xs font-medium text-slate-600"><div>${s.phone || '-'}</div><div class="text-[10px] text-slate-400 mt-0.5">user@email (Hidden)</div></td><td class="text-xs font-bold uppercase text-slate-500">${locName}</td><td class="text-right p-6">${s.status==='suspended'?`<button onclick="window.toggleUserStatus('${s.id}', 'active')" class="text-[9px] bg-green-100 text-green-800 px-3 py-1.5 rounded-full font-bold uppercase">Activate</button>`:`<button onclick="window.openReassignModal('${s.id}', '${s.role}', '${s.assigned_location_id}')" class="text-[9px] bg-red-100 text-red-800 px-3 py-1.5 rounded-full font-bold uppercase hover:bg-red-200 transition">Suspend</button>`}</td></tr>`;
    }).join('')}
    ${inv.map(i => {
        const locName = window.cachedLocations.find(l => l.id === i.assigned_location_id)?.name || 'Unassigned';
        return `<tr class="bg-yellow-50/50 border-b border-yellow-100"><td class="p-6"><div class="font-bold text-sm text-slate-600">${i.email}</div><div class="text-[10px] uppercase font-bold text-slate-400 mt-1">${i.role}</div></td><td class="text-xs text-slate-400 italic">Pending Acceptance</td><td class="text-xs font-bold uppercase text-slate-400">${locName}</td><td class="text-right p-6"><span class="text-[9px] bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full font-bold">PENDING</span></td></tr>`;
    }).join('')}
    </tbody></table></div>`;
};

// 🔥 HAPA NDIPO UCHAWI WA KURUDISHA UWEZO ULIPO
window.toggleUserStatus = async (id, status) => {
    if(id === window.profile.id) return window.showNotification("Cannot suspend self", "error");
    window.premiumConfirm(`${status === 'suspended' ? 'Suspend' : 'Activate'} User?`, `This user will ${status === 'suspended' ? 'lose' : 'regain'} access.`, "Confirm", async () => {
...
window.executeReassign = async () => {
    ...
    // Kama hajaweka email mpya, tunamsuspend tu bila ku-replace
    if(!newEmail) {
...
window.inviteModal = async () => {
    const { data: locs } = await supabase.from('locations').select('*').eq('organization_id', window.profile.organization_id);
    document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-6 uppercase text-center">Invite Staff</h3><div class="input-group"><label class="input-label">Email</label><input id="iE" class="input-field"></div><div class="input-group"><label class="input-label">Role</label><select id="iR" class="input-field"><option value="storekeeper">Storekeeper</option><option value="barman">Barman</option><option value="finance">Finance</option></select></div><div class="input-group"><label class="input-label">Assign</label><select id="iL" class="input-field">${locs.map(l=>`<option value="${l.id}">${l.name}</option>`).join('')}</select></div><button onclick="window.execInvite()" class="btn-primary">SEND INVITE</button>`;
...
// 6. SETTINGS (SUPPLIERS & ALL CURRENCIES)
window.renderSettings = async (c) => {
    const { data: locs } = await supabase.from('locations').select('*').eq('organization_id', window.profile.organization_id);
    const { data: sups } = await supabase.from('suppliers').select('*').eq('organization_id', window.profile.organization_id);
    
    // 🔥 PESA ZOTE ZINAONEKANA NA INPUT
    const rateRows = ALL_CURRENCIES.map(code => {
        const isBase = code === window.baseCurrency;
        const val = isBase ? 1 : (window.currencyRates[code] || '');
        return `<div class="flex justify-between items-center py-2 border-b last:border-0"><span class="font-bold text-xs w-10">${code}</span>
        ${isBase ? '<span class="text-xs font-mono font-bold text-slate-400 bg-slate-50 px-2 py-1 rounded">BASE</span>' : 
        `<input id="rate-${code}" type="number" step="0.01" value="${val}" class="w-24 input-field py-1 text-right font-mono text-xs" placeholder="Rate">`}
        </div>`;
    }).join('');
...
window.saveRates = async () => {
    ...
    ALL_CURRENCIES.forEach(code => {
        ...
        if(el && el.value) {
            updates.push({ organization_id: window.profile.organization_id, currency_code: code, rate: parseFloat(el.value) });
        }
    });
    
    if(updates.length > 0) {
        await supabase.from('exchange_rates').upsert(updates, { onConflict: 'organization_id, currency_code' });
        await window.initCurrency();
...
window.openSupplierModal = async (id = null) => {
    ...
    document.getElementById('supTitle').innerText = id ? 'Edit Supplier' : 'Register Supplier';
    document.getElementById('btnDelSup').style.display = id ? 'block' : 'none'; 
    if (id) {
        const sup = window.cachedSuppliers.find(s => s.id === id);
        if(sup) {
            document.getElementById('supName').value = sup.name;
            document.getElementById('supTIN').value = sup.tin;
            document.getElementById('supPhone').value = sup.contact;
            document.getElementById('supAddr').value = sup.address;
        }
    } else {
        document.getElementById('supName').value = '';
        document.getElementById('supTIN').value = '';
...
window.saveSupplier = async () => {
    ...
    const addr = document.getElementById('supAddr').value;
    const cont = document.getElementById('supPhone').value;
    if(!name) return window.showNotification("Name Required", "error");
    const payload = { organization_id: window.profile.organization_id, name: name.toUpperCase(), tin, address: addr, contact: cont };
...
window.deleteSupplier = async () => {
    const id = document.getElementById('supId').value;
    if(!id) return;
    window.premiumConfirm("Delete Supplier?", "History will remain.", "Delete", async () => {
