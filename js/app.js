...
window.onload = async () => {
    // 1. ZUIA DASHBOARD ISIONEKANE KWANZA (Weka Loading Spinner)
    const app = document.getElementById('app-view');
    if(app) app.innerHTML = '<div class="flex h-screen items-center justify-center"><div class="w-10 h-10 border-4 border-black border-t-transparent rounded-full animate-spin"></div></div>';

    const session = await getSession();
    if (!session) { window.location.href = 'index.html'; return; }

    try {
        // 2. RETRY LOGIC: Jaribu mara 3 kuhakikisha hatukurupuki (Anti-Loop Fix)
        let attempts = 0;
        const maxAttempts = 5; // Jaribu mara 5 (jumla sekunde 2.5)
        
        while (attempts < maxAttempts) {
            profile = await getCurrentProfile(session.user.id);
            
            // Check 1: Kama ana Organization ID (Manager aliyemaliza Setup), Ingia!
            if (profile && profile.organization_id) break;
            
            // Check 2: Kama ana Invite (Staff), Ingia!
            // (Hii inafanyika mara moja tu mwanzo ili tusi-flood database)
            if (attempts === 0) {
                 const { data: invite } = await supabase.from('staff_invites').select('*').eq('email', session.user.email).maybeSingle();
                 if (invite) {
                     // Force Create Profile from Invite
                     const { data: newProf } = await supabase.from('profiles').upsert({
                        id: session.user.id, email: session.user.email, full_name: 'Staff Member',
                        organization_id: invite.organization_id, role: invite.role, assigned_location_id: invite.assigned_location_id
                     }).select().single();
                     await supabase.from('staff_invites').update({ status: 'accepted' }).eq('id', invite.id);
                     profile = newProf;
                     break; // Tumepata mtu, toka kwenye loop
                 }
            }

            // Kama bado hatujapata kitu, subiri 500ms (Nusu sekunde)
            attempts++;
            await new Promise(r => setTimeout(r, 500));
        }

        // 3. MAAMUZI YA MWISHO: Baada ya kusubiri, kama bado hana kitu, nenda Setup
        if (!profile || !profile.organization_id) {
            window.location.href = 'setup.html';
            return;
        }

        // 4. KAMA YUPO SAWA, FUNGUA DASHBOARD
...
window.router = async (view) => {
    const app = document.getElementById('app-view');
    // Spinner ndogo wakati wa kubadili tabs
    ...
    document.getElementById('sidebar').classList.add('-translate-x-full'); // Funga menu ya simu
...
window.inviteModal=()=>{document.getElementById('modal-content').innerHTML=`<h3 class="font-bold mb-2">Invite</h3><form onsubmit="event.preventDefault();window.sI()"><input id="iE" class="input-field w-full mb-2" placeholder="Email"><select id="iR" class="input-field w-full mb-2 bg-white"><option value="storekeeper">Storekeeper</option><option value="barman">Barman</option><option value="finance">Finance</option><option value="dept_user">Department User</option></select><button class="btn-black w-full py-2 rounded">Send</button></form>`;document.getElementById('modal').classList.remove('hidden');}
window.sI=async()=>{try{await inviteStaff(document.getElementById('iE').value, document.getElementById('iR').value, profile.organization_id); alert('Sent'); document.getElementById('modal').classList.add('hidden');}catch(e){alert(e.message);}}
