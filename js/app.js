...
// ============================================================================
// 1. BRANDING & SECURITY (RUNS IMMEDIATELY)
// ============================================================================
(function aggressiveBranding() {
    const targetName = "ugaviSmarT";
    document.title = `${targetName} | Enterprise ERP`;
    
    // Interval check to force change even on Login Screen delays
    setInterval(() => {
        document.querySelectorAll('*').forEach(el => {
            ...
            if (el.childNodes.length === 1 && el.childNodes[0].nodeType === 3) {
                ...
                if (txt.includes('Pilot my Stock') || txt.includes('Pilot')) {
                    el.innerText = txt.replace(/Pilot my Stock|Pilot/g, targetName);
                }
            }
            // Check Placeholders
            if (el.placeholder && el.placeholder.includes('Pilot')) {
                el.placeholder = el.placeholder.replace('Pilot', targetName);
            }
        });
    }, 100);
})();
...
// ============================================================================
// 3. UTILITY FUNCTIONS (Global Scope)
// ============================================================================
...
window.router = async (view) => { 
    ...
    setTimeout(async () => { 
        try { 
            // Explicitly call mapped functions
            switch(view) {
                case 'inventory': await window.renderInventory(app); break;
                case 'bar': await window.renderBar(app); break;
                case 'approvals': await window.renderApprovals(app); break; // Fixed: Function must exist
                case 'reports': await window.renderReports(app); break;
                case 'staff': await window.renderStaff(app); break;
                case 'settings': await window.renderSettings(app); break;
                default: app.innerHTML = "Unknown View";
            }
        } catch (e) { 
            console.error(e); 
...
window.showUpdateProfileModal = () => {
    document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-4 text-center">Complete Profile</h3><div class="input-group"><label>Full Name</label><input id="upName" class="input-field" value="${window.profile.full_name!=='New User'?window.profile.full_name:''}"></div><div class="input-group"><label>Phone</label><input id="upPhone" class="input-field" value="${window.profile.phone||''}"></div><button onclick="window.updateProfile()" class="btn-primary">SAVE</button>`;
...
window.renderInventory = async (c) => {
    ...
    if (isPOView) {
        ...
        content = `<table class="w-full text-left border-collapse"><thead class="bg-slate-50 border-b"><tr><th class="p-4">Date</th><th>Supplier</th><th>Total</th><th>Status</th><th>Action</th></tr></thead><tbody>${(pos||[]).map(p => `<tr class="border-b hover:bg-slate-50"><td class="p-4 font-bold text-slate-500 text-xs">${new Date(p.created_at).toLocaleDateString()}</td><td class="text-sm font-bold uppercase">${p.suppliers?.name || 'Unknown'}</td><td class="font-mono font-bold">${window.formatPrice(p.total_cost)}</td><td><span class="px-2 py-1 rounded text-[10px] font-bold uppercase ${p.status==='Pending'?'bg-yellow-100 text-yellow-700':p.status==='Partial'?'bg-blue-100 text-blue-700':'bg-green-100 text-green-700'}">${p.status}</span></td><td>${p.status!=='Received'?`<button onclick="window.openReceiveModal('${p.id}')" class="text-[10px] bg-slate-900 text-white px-3 py-1 rounded font-bold">RECEIVE</button>`:'<span class="text-xs text-slate-300 font-bold">DONE</span>'}</td></tr>`).join('')}</tbody></table>`;
    } else {
        content = `<table class="w-full text-left border-collapse"><thead class="bg-slate-50 border-b"><tr><th class="p-4">Item</th>${showPrice?'<th>Cost</th>':''}<th>Store</th><th>Stock</th><th>Action</th></tr></thead><tbody>${filteredStock.map(i => `<tr class="border-b hover:bg-slate-50"><td class="p-4"><div class="font-bold text-sm uppercase">${i.products?.name}</div><div class="text-[10px] text-slate-400">${i.products?.category}</div></td>${showPrice?`<td class="font-mono text-xs">${window.formatPrice(i.products.cost_price)}</td>`:''} <td class="text-xs font-bold text-slate-500 uppercase">${i.locations?.name}</td><td class="font-mono font-bold text-lg">${i.quantity} <span class="text-[10px] text-slate-400">${i.products?.unit}</span></td><td class="flex gap-2 p-4 justify-end">${window.profile.role!=='barman'?`<button onclick="window.issueModal('${i.products.name}','${i.product_id}','${i.location_id}')" class="text-[10px] border px-2 py-1 rounded hover:bg-slate-900 hover:text-white">MOVE</button>`:''}${canAdjust?`<button onclick="window.openStockEdit('${i.id}', '${i.quantity}')" class="text-[10px] border px-2 py-1 rounded hover:bg-slate-100">EDIT</button><button onclick="window.requestDeleteProduct('${i.products.id}')" class="text-[10px] border px-2 py-1 rounded hover:bg-red-50 text-red-500">DEL</button>`:''}</td></tr>`).join('')}</tbody></table>`;
    }
    
    c.innerHTML = `<div class="flex flex-col md:flex-row justify-between items-center gap-6 mb-8"><div class="flex items-center gap-4"><h1 class="text-3xl font-bold uppercase text-slate-900">Inventory</h1>${showPrice?window.getCurrencySelectorHTML():''}</div><div class="flex gap-1 bg-slate-100 p-1 rounded-xl"><button onclick="window.currentInvView='stock'; window.router('inventory')" class="px-6 py-2 text-xs font-bold rounded-lg ${!isPOView?'bg-white shadow':''}">STOCK</button><button onclick="window.currentInvView='po'; window.router('inventory')" class="px-6 py-2 text-xs font-bold rounded-lg ${isPOView?'bg-white shadow':''}">LPO</button></div><div class="flex gap-3">${canCreateLPO?`<button onclick="window.createPOModal()" class="btn-primary w-auto px-6 shadow-lg bg-blue-600">LPO</button><button onclick="window.addProductModal()" class="btn-primary w-auto px-6 shadow-lg">NEW ITEM</button>`:''}</div></div><div class="bg-white rounded-3xl border shadow-sm overflow-hidden min-h-[400px]">${content}</div>`;
};
...
window.createPOModal = async () => { 
    ...
    document.getElementById('modal-content').innerHTML = `
        ...
        <div class="input-group mb-4"><label>Supplier</label><select id="lpoSup" class="input-field font-bold">${sups.map(s => `<option value="${s.id}">${s.name}</option>`).join('')}</select></div>
        <div class="bg-slate-50 p-4 rounded-xl border mb-6 max-h-[300px] overflow-y-auto custom-scrollbar">${prods.map(p => `<div class="flex items-center gap-3 bg-white p-3 rounded-lg border mb-2"><input type="checkbox" class="lpo-check w-5 h-5" value="${p.id}" data-price="${p.cost_price}"><div class="flex-1"><span class="block text-xs font-bold uppercase">${p.name}</span><span class="text-[10px] text-slate-400">Cost: ${window.formatPrice(p.cost_price)}</span></div><input type="number" id="qty-${p.id}" class="w-20 input-field py-1 text-center font-bold" placeholder="Qty"></div>`).join('')}</div>
        <button onclick="window.execCreatePO()" class="btn-primary w-full">CREATE LPO</button>`; 
    document.getElementById('modal').style.display = 'flex'; 
};
...
window.openReceiveModal = async (poId) => {
    ...
    document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-2 text-center">Receive Stock (GRN)</h3><p class="text-xs text-center text-slate-400 mb-4">PO: ${poId.split('-')[0]}</p><div class="mb-6 max-h-[300px] overflow-y-auto">${itemRows}</div><button onclick="window.execReceivePO('${poId}')" class="btn-primary w-full bg-blue-600">CONFIRM RECEIPT</button>`;
    document.getElementById('modal').style.display = 'flex';
};
...
window.addProductModal = () => { 
    ...
    document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-6 uppercase text-center">New Product</h3><div class="input-group"><label>Name</label><input id="pN" class="input-field uppercase"></div><div class="grid grid-cols-2 gap-4 mb-4"><div><label>Category</label><select id="pCat" class="input-field">${catOpts}</select></div><div><label>Unit</label><select id="pUnit" class="input-field">${unitOpts}</select></div></div><div class="input-group mb-4"><label>Currency</label><select id="pCurrency" class="input-field">${currOpts}</select></div><div class="grid grid-cols-2 gap-4 mb-6"><div><label>Cost</label><input id="pC" type="number" class="input-field"></div><div><label>Selling</label><input id="pS" type="number" class="input-field"></div></div><button onclick="window.nextProductStep()" class="btn-primary">Next: Prices</button>`; 
    document.getElementById('modal').style.display = 'flex'; 
};
window.nextProductStep = async () => {
    ...
    document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-4 text-center">Camp Pricing</h3><div class="mb-6 max-h-60 overflow-y-auto">${html}</div><button onclick="window.finalizeProduct()" class="btn-primary">SAVE</button>`;
};
...
window.renderBar = async (c) => {
    ...
    const storeSelect = (window.profile.role !== 'barman') ? `<div class="mb-8 flex items-center gap-4"><span class="text-xs font-bold text-slate-400 uppercase">Active Counter:</span><select onchange="window.switchBar(this.value)" class="bg-white border rounded-xl px-4 py-2 font-bold">${locs.map(l => `<option value="${l.id}" ${window.activePosLocationId===l.id?'selected':''}>${l.name}</option>`).join('')}</select></div>` : '';
    const payMethods = ['Cash', 'Mobile Money', 'Credit Card', 'Room Charge'].map(m => `<button onclick="window.setPaymentMethod('${m}')" class="pay-btn flex-1 py-2 text-[10px] font-bold uppercase rounded-lg border transition ${window.selectedPaymentMethod === m ? 'bg-slate-900 text-white' : 'bg-white text-slate-500'}">${m}</button>`).join('');
    c.innerHTML = `${storeSelect} <div class="flex flex-col lg:flex-row gap-8 h-[calc(100vh-140px)]"><div class="flex-1 overflow-y-auto pr-2"><div class="flex justify-between items-center mb-6 sticky top-0 bg-[#F8FAFC] py-2 z-10"><h1 class="text-3xl font-bold uppercase text-slate-900">POS Terminal</h1>${window.getCurrencySelectorHTML()}</div><div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 pb-20">${items.length ? items.map(x => `<div onclick="window.addCart('${x.products.name}', ${x.products.selling_price}, '${x.product_id}')" class="bg-white p-5 rounded-2xl border cursor-pointer hover:border-slate-900 hover:shadow-lg transition group"><div class="flex justify-between items-start mb-2"><div class="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-300 font-bold text-[10px] group-hover:bg-slate-900 group-hover:text-white">${x.products.name.charAt(0)}</div><span class="text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-1 rounded-full">Qty: ${x.quantity}</span></div><p class="text-xs font-bold uppercase mb-1 truncate">${x.products.name}</p><p class="text-lg font-bold font-mono">${window.formatPrice(x.products.selling_price)}</p></div>`).join('') : '<div class="col-span-full py-20 text-center border-2 border-dashed border-slate-200 rounded-3xl"><p class="text-slate-400 font-bold text-sm">No items available.</p></div>'}</div></div><div class="w-full lg:w-96 bg-white border rounded-[32px] p-8 h-full flex flex-col shadow-2xl"><div class="flex justify-between items-center mb-6"><h3 class="font-bold text-sm uppercase">Order</h3><button onclick="window.cart=[];window.renderCart()" class="text-[10px] font-bold text-red-500">CLEAR</button></div><div id="cart-list" class="flex-1 overflow-y-auto space-y-3 pr-1"></div><div class="pt-6 border-t mt-auto"><div class="mb-4"><p class="text-[10px] font-bold text-slate-400 uppercase mb-2">Payment</p><div class="flex gap-2 flex-wrap">${payMethods}</div></div><div class="flex justify-between items-end mb-6"><span class="text-xs font-bold text-slate-400 uppercase">Total</span><span id="cart-total" class="text-3xl font-bold font-mono">${window.formatPrice(0)}</span></div><button onclick="window.confirmCheckout()" class="w-full bg-[#0F172A] text-white py-4 rounded-2xl font-bold text-sm uppercase tracking-widest hover:bg-slate-800">CHARGE SALE</button></div></div></div>`;
    window.renderCart();
};
...
window.renderCart = () => { const l=document.getElementById('cart-list'), t=document.getElementById('cart-total'); let sum=0; l.innerHTML=(window.cart||[]).map(i=>{sum+=i.price*i.qty; return `<div class="flex justify-between items-center p-3 bg-slate-50 rounded-xl border group"><div class="flex flex-col"><span class="text-xs font-bold uppercase">${i.name}</span><span class="text-[10px] text-slate-400 font-mono">${window.formatPrice(i.price)} x ${i.qty}</span></div><button onclick="window.remCart('${i.id}')" class="w-6 h-6 flex items-center justify-center rounded-full text-slate-300 hover:text-red-500">✕</button></div>`}).join(''); t.innerText=window.formatPrice(sum); };
window.remCart = (id) => { window.cart=window.cart.filter(c=>c.id!==id); window.renderCart(); };
window.confirmCheckout = () => { if(!window.cart.length) return; window.premiumConfirm(`Confirm ${window.selectedPaymentMethod} Sale?`, "Charge this amount.", "Charge", window.doCheckout); };
window.doCheckout = async () => { try { await processBarSale(window.profile.organization_id, window.activePosLocationId, window.cart.map(c=>({product_id:c.id,qty:c.qty,price:c.price})), window.profile.id, window.selectedPaymentMethod); window.showNotification("Sale Successful", "success"); window.cart=[]; window.renderCart(); window.router('bar'); } catch(e) { window.showNotification(e.message, "error"); } };

// ============================================================================
// 7. APPROVALS (RENDERAPPROVALS DEFINED)
// ============================================================================
window.renderApprovals = async (c) => {
    if(window.profile.role === 'storekeeper' || window.profile.role === 'barman') return c.innerHTML = '<div class="p-20 text-center text-slate-400">Restricted Area</div>';
    ...
    c.innerHTML = `
    <h1 class="text-3xl font-bold mb-8 uppercase text-slate-900">Approvals</h1>
    <div class="mb-8"><h3 class="text-sm font-bold text-slate-400 uppercase mb-4">Transfers</h3><div class="bg-white rounded-3xl border shadow-sm overflow-hidden"><table class="w-full text-left"><tbody>${reqs.length ? reqs.map(r => `<tr class="border-b hover:bg-slate-50"><td class="p-6"><div class="font-bold text-sm uppercase">${r.products?.name}</div><div class="text-xs text-slate-400 uppercase">From: ${r.from_loc?.name}</div></td><td class="p-6"><div class="text-blue-600 font-mono font-bold text-lg">${r.quantity}</div><div class="text-xs text-slate-400 uppercase">Qty</div></td><td class="p-6 text-xs font-bold text-slate-500 uppercase">To: ${r.to_loc?.name}</td><td class="p-6 text-right"><button onclick="window.confirmApprove('${r.id}')" class="text-[10px] bg-slate-900 text-white px-6 py-3 rounded-xl font-bold shadow-lg">AUTHORIZE</button></td></tr>`).join('') : '<tr><td colspan="4" class="p-8 text-center text-xs font-bold text-slate-300">No requests.</td></tr>'}</tbody></table></div></div>
    <div><h3 class="text-sm font-bold text-slate-400 uppercase mb-4">Admin Changes</h3><div class="bg-white rounded-3xl border shadow-sm overflow-hidden"><table class="w-full text-left"><tbody>${changes && changes.length ? changes.map(r => `<tr class="border-b hover:bg-slate-50"><td class="p-6"><div class="font-bold text-sm uppercase text-red-600">${r.action}</div><div class="text-xs text-slate-400 uppercase">By: ${r.requester?.full_name}</div></td><td class="p-6 text-xs font-mono text-slate-500">${new Date(r.created_at).toLocaleString()}</td><td class="p-6 text-right"><button onclick="window.approveChange('${r.id}')" class="text-[10px] bg-red-600 text-white px-6 py-3 rounded-xl font-bold shadow-lg">APPROVE</button></td></tr>`).join('') : '<tr><td colspan="3" class="p-8 text-center text-xs font-bold text-slate-300">No admin requests.</td></tr>'}</tbody></table></div></div>`;
};
...
window.approveChange = (id) => { window.premiumConfirm("Approve Change?", "Execute request.", "Approve", async () => { await supabase.rpc('process_change_request', { p_request_id: id, p_status: 'approved', p_reviewer_id: window.profile.id }); window.showNotification("Executed", "success"); window.router('approvals'); }); };

// ============================================================================
// 8. STAFF MODULE
// ============================================================================
window.renderStaff = async (c) => {
    if(!['manager', 'financial_controller'].includes(window.profile.role)) return c.innerHTML = '<div class="p-20 text-center">Restricted</div>';
    const [staff, invites] = await Promise.all([supabase.from('profiles').select('*, locations(name)').eq('organization_id', window.profile.organization_id), supabase.from('staff_invites').select('*, locations(name)').eq('organization_id', window.profile.organization_id).eq('status', 'pending')]);
    c.innerHTML = `<div class="flex justify-between items-center mb-8"><h1 class="text-3xl font-bold uppercase">Team</h1><button onclick="window.inviteModal()" class="btn-primary w-auto px-6">INVITE</button></div><div class="bg-white rounded-3xl border shadow-sm overflow-hidden mb-8"><table class="w-full text-left"><thead class="bg-slate-50 border-b"><tr><th class="p-4 text-xs text-slate-400 uppercase">User</th><th class="text-xs text-slate-400 uppercase">Role</th><th class="text-xs text-slate-400 uppercase">Loc</th><th class="text-xs text-slate-400 uppercase text-right pr-6">Action</th></tr></thead><tbody>${(staff.data||[]).map(s => `<tr class="border-b last:border-0 hover:bg-slate-50 transition"><td class="p-4 font-bold text-slate-700">${s.full_name||'Pending'}<br><span class="text-[10px] text-slate-400 font-normal">${s.email}</span></td><td class="text-xs uppercase">${s.role.replace('_', ' ')}</td><td class="text-xs uppercase">${s.locations?.name||'Global'}</td><td class="text-right pr-6 flex justify-end gap-2 mt-3"><button onclick="window.viewStaffDetails('${s.id}')" class="text-[10px] border px-2 py-1 rounded hover:bg-slate-100">LOGS</button>${s.id!==window.profile.id?`<button onclick="window.toggleSuspend('${s.id}', '${s.status}')" class="text-[10px] border px-2 py-1 rounded ${s.status==='active'?'text-red-500':'text-green-500'}">${s.status==='active'?'SUSPEND':'ACTIVATE'}</button>`:''}</td></tr>`).join('')}</tbody></table></div>`;
};
window.inviteModal = () => {
    const locOpts = window.cachedLocations.map(l => `<option value="${l.id}">${l.name}</option>`).join('');
    document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-6 text-center">Invite Staff</h3><div class="input-group"><label>Email</label><input id="iE" type="email" class="input-field"></div><div class="grid grid-cols-2 gap-4 mb-4"><div><label>Role</label><select id="iR" class="input-field" onchange="window.toggleLocSelect(this.value)"><option value="storekeeper">Storekeeper</option><option value="deputy_storekeeper">Deputy Storekeeper</option><option value="barman">Barman</option><option value="finance">Finance</option><option value="financial_controller">Controller</option><option value="deputy_manager">Deputy Manager</option></select></div><div id="locDiv"><label>Assign To</label><select id="iL" class="input-field">${locOpts}</select></div></div><button onclick="window.execInvite()" class="btn-primary">SEND</button>`;
    document.getElementById('modal').style.display = 'flex';
};
window.toggleLocSelect = (role) => { document.getElementById('locDiv').style.display = role === 'financial_controller' ? 'none' : 'block'; };
window.execInvite = async () => { const email=document.getElementById('iE').value, role=document.getElementById('iR').value, loc=role==='financial_controller'?null:document.getElementById('iL').value; if(!email)return; await supabase.from('staff_invites').insert({organization_id:window.profile.organization_id, email, role, assigned_location_id:loc, status:'pending'}); document.getElementById('modal').style.display='none'; window.renderStaff(document.getElementById('app-view')); };
window.viewStaffDetails = async (id) => { const [l,p] = await Promise.all([supabase.from('audit_logs').select('*').eq('user_id', id).limit(5), supabase.from('profiles').select('*').eq('id', id).single()]); document.getElementById('modal-content').innerHTML=`<h3 class="text-center font-bold mb-4">${p.data.full_name}</h3><div class="bg-slate-50 p-4 rounded text-xs">${l.data.map(x=>`<div class="border-b py-1"><b>${x.action}</b> ${new Date(x.created_at).toLocaleDateString()}</div>`).join('')}</div>`; document.getElementById('modal').style.display='flex'; };
...
// ============================================================================
// 9. REPORTS (ADDED LOCATION FILTER)
// ============================================================================
    ...
    if(isVariance) {
        ...
        <div class="flex flex-col gap-6 mb-8">
            ...
            <div class="bg-white p-4 rounded-xl border flex gap-4 flex-wrap items-end shadow-sm">
                ...
                <div><label class="input-label">Location/Camp</label><select id="rLoc" class="input-field w-32" onchange="window.filterReport()"><option value="all">All Camps</option>${locOpts}</select></div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-6"><div class="bg-white p-6 border rounded-xl"><span class="text-xs text-slate-400 font-bold uppercase">Revenue</span><div id="repRev" class="text-3xl font-bold">0.00</div></div><div class="bg-white p-6 border rounded-xl"><span class="text-xs text-slate-400 font-bold uppercase">Transactions</span><div id="repCount" class="text-3xl font-bold">0</div></div></div>
...
window.filterReport = () => {
    ...
    if(loc && loc !== 'all') f = f.filter(l => l.to_location_id === loc || l.from_location_id === loc); // Filter by Location
    ...
    document.getElementById('repCount').innerText = f.length;
...
window.addStoreModal = () => { const parents = window.cachedLocations.filter(l => l.type !== 'department').map(l => `<option value="${l.id}">${l.name}</option>`).join(''); document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-6 uppercase text-center">Add Location</h3><div class="input-group"><label>Name</label><input id="locName" class="input-field uppercase"></div><div class="input-group"><label>Type</label><select id="locType" class="input-field" onchange="document.getElementById('parentGrp').style.display = this.value === 'main_store' ? 'none' : 'block'"><option value="sub_store">Camp / Sub-Store</option><option value="department">Department</option><option value="main_store">Main Store</option></select></div><div id="parentGrp" class="input-group"><label>Parent</label><select id="locParent" class="input-field">${parents}</select></div><button onclick="window.execAddStore()" class="btn-primary">CREATE</button>`; document.getElementById('modal').style.display = 'flex'; };
window.execAddStore = async () => { const name = document.getElementById('locName').value; const type = document.getElementById('locType').value; const parent = type === 'main_store' ? null : document.getElementById('locParent').value; if(!name) return; await supabase.from('locations').insert({ organization_id: window.profile.organization_id, name, type, parent_location_id: parent }); document.getElementById('modal').style.display = 'none'; window.showNotification("Created", "success"); window.router('settings'); };
window.openSupplierModal = () => { document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-6 uppercase text-center">New Supplier</h3><div class="input-group"><label>Name</label><input id="sN" class="input-field uppercase"></div><div class="input-group"><label>TIN</label><input id="sT" class="input-field"></div><div class="input-group"><label>Phone</label><input id="sP" class="input-field"></div><div class="input-group"><label>Address</label><input id="sA" class="input-field"></div><button onclick="window.execAddSupplier()" class="btn-primary">SAVE</button>`; document.getElementById('modal').style.display = 'flex'; };
window.execAddSupplier = async () => { const name = document.getElementById('sN').value; if(!name) return; await supabase.from('suppliers').insert({ organization_id: window.profile.organization_id, name, tin: document.getElementById('sT').value, contact: document.getElementById('sP').value, address: document.getElementById('sA').value }); document.getElementById('modal').style.display = 'none'; window.showNotification("Saved", "success"); window.router('settings'); };
