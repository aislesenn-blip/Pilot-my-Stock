...
window.initCurrency = async function() {
    ...
    try {
        ...
        const { data: rates, error: rateError } = await supabase.from('exchange_rates').select('*').eq('organization_id', window.profile.organization_id);
        if (rateError) throw rateError;

        window.currencyRates = { [window.baseCurrency]: 1 };
        (rates||[]).forEach(r => window.currencyRates[r.currency_code] = Number(r.rate));
    } catch(e){
        handleError(e, "Init Currency");
    }
};
...
window.onload = async function() {
    try {
        ...
        const hide = (ids) => ids.forEach(id => {
            ...
            if(el) el.remove(); // REMOVE from DOM entirely
        });

        // 1. Barman: LOCKED to POS
        if (role === 'barman') {
            document.getElementById('sidebar').style.display = 'none'; // Hide sidebar
            document.querySelector('main').classList.remove('md:ml-72'); // Full width
            document.querySelector('.md:hidden').style.display = 'none'; // Hide mobile header
            window.router('bar');
            return; // Stop execution of other role logic
        }

        // 2. Storekeeper: Inventory Only
        if (role === 'storekeeper' || role === 'deputy_storekeeper') {
            hide(['nav-bar', 'nav-approvals', 'nav-staff', 'nav-settings']);
            window.router('inventory');
        }
        // 3. Finance: No Staff, No Settings
        else if (role === 'finance' || role === 'deputy_finance') {
            hide(['nav-bar', 'nav-settings', 'nav-staff']);
            window.router('inventory');
        }
        // 4. Financial Controller: No POS (Conflict of Interest)
...
window.router = async function(view) {
    // Double Check Lock for Barman (In case they try console hacks)
...
window.saveName = async function() {
    try {
        ...
        location.reload();
    } catch (e) { handleError(e, "Save Name"); }
};
...
window.updateProfile = async function() {
    ...
    } catch (e) { handleError(e, "Update Profile"); }
};
...
window.renderInventory = async function(c) {
    try {
        ...
        // RBAC: Storekeeper only sees their store
        if (window.profile.role.includes('storekeeper') && !window.profile.role.includes('overall')) {
             const { data, error } = await supabase.from('inventory').select('*, products(*), locations(name)').eq('organization_id', window.profile.organization_id).eq('location_id', window.profile.assigned_location_id);
             if(error) throw error;
             stock = data;
        } else {
        ...
        const canAdjust = ['manager', 'deputy_manager', 'financial_controller', 'overall_storekeeper'].includes(window.profile.role);
...
// ... (LPO Create Logic kept from previous, just wrapped) ...
...
window.execCreatePO = async function() {
    ...
    } catch(e) { handleError(e, "Exec Create PO"); }
};
...
window.execReceivePO = async function(poId) {
    const items = [];
    document.querySelectorAll('.rec-inp').forEach(i => { if(i.value > 0) items.push({ po_item_id: i.dataset.id, product_id: i.dataset.pid, qty: parseFloat(i.value) }); });
...
window.openInviteModal = function() {
    // Populate locations dynamically even if modal is static
...
window.execInvite = async function() {
    try {
        ...
        if (role === 'financial_controller') locId = null; // FC is Global
...
window.renderReports = async function(c) {
    try {
        ...
        // P&L Logic: Use new 'gross_profit' column if available, else calc
        const revenue = window.currentLogs.filter(l => l.type === 'sale').reduce((sum, l) => sum + (l.total_value || 0), 0);
        const profit = window.currentLogs.filter(l => l.type === 'sale').reduce((sum, l) => sum + (l.gross_profit || 0), 0);

        c.innerHTML = `
        <h1 class="text-3xl font-bold mb-8">P&L Report</h1>
        <div class="grid grid-cols-2 gap-4 mb-6">
            <div class="bg-white p-6 border rounded-xl"><span class="text-xs text-slate-400 font-bold uppercase">Total Revenue</span><div class="text-3xl font-bold">${window.formatPrice(revenue)}</div></div>
            <div class="bg-white p-6 border rounded-xl"><span class="text-xs text-slate-400 font-bold uppercase">Gross Profit</span><div class="text-3xl font-bold text-green-600">${window.formatPrice(profit)}</div></div>
        </div>
        <div class="bg-white rounded-xl border overflow-hidden">
            <table class="w-full text-left">
                <thead class="bg-slate-50"><tr><th class="p-4">Date</th><th>Route</th><th>Item</th><th>Snapshot Cost</th><th>Profit</th></tr></thead>
                <tbody>${window.currentLogs.map(l => `
                    <tr class="border-b hover:bg-slate-50">
                        <td class="p-4 text-xs font-bold text-slate-500">${new Date(l.created_at).toLocaleDateString()}</td>
                        <td class="text-xs font-bold uppercase">${l.from_loc?.name || 'Unknown'} ➝ ${l.type === 'sale' ? 'Client' : l.locations?.name}</td>
                        <td class="text-xs">${l.products?.name}</td>
                        <td class="text-xs font-mono text-slate-400">${window.formatPrice(l.unit_cost_snapshot)}</td>
                        <td class="text-xs font-mono font-bold text-green-600">${window.formatPrice(l.gross_profit)}</td>
                    </tr>`).join('')}
                </tbody>
            </table>
        </div>`;
...
window.renderSettings = async (c) => {
    try {
        ...
        window.cachedLocations = l.data || []; window.cachedSuppliers = s.data || [];
...
window.issueModal = async (name, id, fromLoc) => { try { window.selectedDestinationId = null; const { data: locs, error } = await supabase.from('locations').select('*').eq('organization_id', window.profile.organization_id).neq('id', fromLoc); if(error) throw error; const html = locs.map(l => `<div onclick="window.selectDest(this, '${l.id}')" class="dest-card border p-3 rounded cursor-pointer hover:bg-slate-50 text-center"><span class="font-bold text-xs uppercase">${l.name}</span></div>`).join(''); document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-4 uppercase text-center">Move: ${name}</h3><div class="grid grid-cols-2 gap-2 mb-4 max-h-40 overflow-y-auto">${html}</div><div class="input-group"><label class="input-label">Quantity</label><input id="tQty" type="number" class="input-field"></div><button onclick="window.execIssue('${id}', '${fromLoc}')" class="btn-primary">TRANSFER</button>`; document.getElementById('modal').style.display = 'flex'; } catch(e) { handleError(e, "Issue Modal"); } };
...
window.execAddStore = async () => { try { const name = document.getElementById('locName').value; const type = document.getElementById('locType').value; const parent = type === 'main_store' ? null : document.getElementById('locParent').value; if(!name) return; await createLocation(window.profile.organization_id, name, type, parent); document.getElementById('modal').style.display = 'none'; window.showNotification("Location Created", "success"); window.router('settings'); } catch(e) { handleError(e, "Exec Add Store"); } };
...
window.execAddSupplier = async () => { try { const name = document.getElementById('sN').value; if(!name) return; const { error } = await supabase.from('suppliers').insert({ organization_id: window.profile.organization_id, name, tin: document.getElementById('sT').value, contact: document.getElementById('sP').value, address: document.getElementById('sA').value }); if(error) throw error; document.getElementById('modal').style.display = 'none'; window.showNotification("Supplier Saved", "success"); window.router('settings'); } catch(e) { handleError(e, "Exec Add Supplier"); } };
