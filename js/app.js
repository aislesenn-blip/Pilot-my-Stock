...
import { getInventory, getLocations, createLocation, processBarSale, getPendingApprovals, respondToApproval, transferStock } from './services.js';
...
function applyPermissions(role) {
    const menus = { inventory: document.getElementById('nav-inventory'), bar: document.getElementById('nav-bar'), approvals: document.getElementById('nav-approvals'), staff: document.getElementById('nav-staff'), settings: document.getElementById('nav-settings') };
    if (role === 'manager') return;
    if (role === 'finance') { ["bar", "staff", "settings"].forEach(id => menus[id]?.classList.add('hidden')); }
    else if (role === 'barman') { ["approvals", "staff", "settings"].forEach(id => menus[id]?.classList.add('hidden')); }
    else if (role === 'storekeeper') { ["bar", "approvals", "staff", "settings"].forEach(id => menus[id]?.classList.add('hidden')); }
}
...
async function renderInventory(c) {
    try {
        ...
        const stockRows = filteredStock.map(i => `<tr class="border-b border-gray-50"><td class="p-4 font-bold text-sm uppercase">${i.products?.name}</td><td class="p-4 text-[10px] font-bold text-gray-400 uppercase">${i.locations?.name}</td><td class="p-4 font-mono font-bold text-gray-900">${i.quantity}</td><td class="p-4 text-right">${profile.role !== 'barman' ? `<button onclick="window.issueModal('${i.products.name}','${i.product_id}','${i.location_id}')" class="text-[10px] font-bold bg-black text-white px-3 py-2 rounded-xl">TRANSFER</button>` : ''}</td></tr>`).join('');
        ...
        c.innerHTML = `
            <div class="flex justify-between items-center mb-10"><h1 class="text-2xl font-bold uppercase tracking-tight">Inventory</h1><div class="flex gap-2">
                <button onclick="window.addProductModal()" class="border border-black px-4 py-2.5 rounded-xl text-[10px] font-bold uppercase">Register Product</button>
            ...
            <h3 class="text-[10px] font-bold text-gray-400 uppercase mb-4 tracking-widest">1. Storage Balances</h3>
            <div class="bg-white rounded-2xl border overflow-hidden shadow-sm mb-12"><table class="w-full text-left"><thead><tr class="bg-gray-50 text-[10px] uppercase font-bold text-gray-400 border-b"><th class="p-4">SKU</th><th class="p-4">Store</th><th class="p-4">Qty</th><th class="p-4 text-right">Action</th></tr></thead><tbody>${stockRows.length ? stockRows : '<tr><td colspan="4" class="p-12 text-center text-xs text-gray-400 font-bold uppercase">No physical records.</td></tr>'}</tbody></table></div>
            <h3 class="text-[10px] font-bold text-gray-400 uppercase mb-4 tracking-widest">2. Master Product Catalog</h3>
            <div class="bg-white rounded-2xl border overflow-hidden shadow-sm"><table class="w-full text-left"><thead><tr class="bg-gray-50 text-[10px] uppercase font-bold text-gray-400 border-b"><th class="p-4">Catalog Item</th><th class="p-4">Standard Pricing</th><th class="p-4 text-right">Status</th></tr></thead><tbody>${catalogRows.length ? catalogRows : '<tr><td colspan="3" class="p-10 text-center text-xs text-gray-300 font-bold uppercase">Catalog empty.</td></tr>'}</tbody></table></div>`;
    } catch(e) {}
}
...
window.addStockModal = async () => {
    ...
    document.getElementById('modal-content').innerHTML = `
        <h3 class="font-bold text-lg mb-8 uppercase tracking-tight">Stock Reception</h3>
        <div class="modal-body" style="overflow: visible !important;">
            <div class="layer-1"><label class="text-[10px] font-bold text-gray-400 uppercase mb-1 block">1. Select Product</label><select id="sP" class="input-field">${prods.map(p=>`<option value="${p.id}">${p.name.toUpperCase()}</option>`).join('')}</select></div>
            <div class="layer-2"><label class="text-[10px] font-bold text-gray-400 uppercase mb-1 block">2. Target Store</label><select id="sL" class="input-field">${locs.map(l=>`<option value="${l.id}">${l.name.toUpperCase()}</option>`).join('')}</select></div>
            <div><label class="text-[10px] font-bold text-gray-400 uppercase mb-1 block">3. Quantity</label><input id="sQ" type="number" class="input-field" placeholder="0.00"></div>
...
window.addProductModal = () => {
    document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-8 uppercase tracking-tight">Register Product</h3><div class="modal-body"><input id="pN" class="input-field uppercase" placeholder="PRODUCT NAME"><div class="grid grid-cols-2 gap-4"><input id="pC" type="number" class="input-field" placeholder="COST"><input id="pS" type="number" class="input-field" placeholder="RETAIL"></div><button onclick="window.execAddProduct()" class="btn-black w-full py-4 text-[11px] mt-2">Finalize Registry</button></div>`;
    document.getElementById('modal').style.display = 'flex';
}
...
async function renderApprovals(c) {
    ...
    const rows = q.map(x => `<tr class="border-b"><td class="p-4 font-bold text-sm uppercase">${x.products?.name}</td><td class="p-4 font-bold text-blue-600">${x.quantity}</td><td class="p-4 text-right"><button onclick="window.approve('${x.id}','approved')" class="bg-black text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase">Authorize</button></td></tr>`).join('');
    c.innerHTML = `<h1 class="text-xl font-bold mb-10 uppercase text-gray-400">Approvals Center</h1><div class="bg-white rounded-2xl border shadow-sm overflow-hidden"><table class="w-full text-left"><tbody>${rows.length?rows:'<tr><td class="p-12 text-center text-xs text-gray-400 font-bold uppercase">No pending items.</td></tr>'}</tbody></table></div>`;
}
...
async function renderSettings(c) {
    ...
    const rows = locs.map(l => `<tr class="border-b"><td class="p-4 font-bold text-sm uppercase">${l.name}</td><td class="p-4 uppercase text-[10px] font-bold text-gray-400">${l.type}</td><td class="p-4 text-right text-green-600 font-bold text-[9px] uppercase tracking-tighter">Active Store</td></tr>`).join('');
    c.innerHTML = `<h1 class="text-xl font-bold mb-8 uppercase text-gray-400">Hub Configuration</h1><div class="bg-white rounded-2xl border shadow-sm overflow-hidden"><table class="w-full text-left"><tbody>${rows}</tbody></table></div>`;
}
