...
window.onload = async () => {
    // 1. Loading UI - Safi na Simple
    const app = document.getElementById('app-view');
    if(app) app.innerHTML = '<div class="flex h-screen items-center justify-center flex-col"><div class="w-10 h-10 border-4 border-black border-t-transparent rounded-full animate-spin mb-4"></div><p class="text-xs font-bold text-gray-400 tracking-widest">LOADING SYSTEM...</p></div>';
    ...
    try {
        ...
        // Trigger imeshafanya kazi yake hapa kama ni Staff aliyealikwa.
        profile = await getCurrentProfile(session.user.id);

        // 3. UAMUZI (DECISION LOGIC)
        if (!profile || !profile.organization_id) {
            
            // EMERGENCY CHECK: Kama Trigger ilichelewa, angalia Invite kwa mkono
            const { data: invite } = await supabase.from('staff_invites').select('*').eq('email', session.user.email).maybeSingle();
            
            if (invite) {
                // FORCE STAFF ENTRY (Bypass Setup)
                console.log("Invite Found - Bypassing Setup");
                ...
                // HANA INVITE, HANA ORG ID -> NENDA SETUP
                console.log("New Manager - Redirecting to Setup");
...
// --- CORE FUNCTIONS (Copy zilezile ulizonazo, hazijabadilika) ---
// Nimezifupisha hapa ili usichanganyike, TUMIA ZILE ZILE ZA FUNCTION ZILIZOPO
