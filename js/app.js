...
async function renderInventory(c) {
    try {
        ...
        const stockRows = filteredStock.map(i => `<tr><td class="font-bold text-gray-800 uppercase">${i.products?.name}</td><td class="text-xs font-bold text-gray-500 uppercase tracking-widest">${i.locations?.name}</td><td class="font-mono font-bold text-gray-900">${i.quantity}</td><td class="text-right">${profile.role !== 'barman' ? `<button onclick="window.issueModal('${i.products.name}','${i.product_id}','${i.location_id}')" class="text-[10px] font-bold bg-slate-900 text-white px-3 py-1.5 rounded hover:bg-slate-700">MOVE</button>` : ''}</td></tr>`).join('');
        ...
        c.innerHTML = `
            ...
            <div class="mb-10"><h3 class="text-xs font-bold text-gray-400 uppercase mb-4 tracking-widest">1. Physical Stock Balance</h3><div class="table-container"><div class="table-scroll"><table><thead><tr><th>Item</th><th>Location</th><th>Qty</th><th class="text-right">Action</th></tr></thead><tbody>${stockRows.length ? stockRows : '<tr><td colspan="4" class="text-center text-xs text-gray-400 py-8">No stock available.</td></tr>'}</tbody></table></div></div></div>
            <div><h3 class="text-xs font-bold text-gray-400 uppercase mb-4 tracking-widest">2. Master Product Catalog</h3><div class="table-container"><div class="table-scroll"><table><thead><tr><th>Product Name</th><th>Standard Pricing</th><th class="text-right">Status</th></tr></thead><tbody>${catalogRows}</tbody></table></div></div></div>`;
    } catch(e) { console.error(e); }
}

// 2. REPORTS (Logic Fixed)
...
async function renderStaff(c) {
    ...
    c.innerHTML = `<div class="flex justify-between items-center mb-8"><h1 class="text-2xl font-bold uppercase text-gray-900">Team Management</h1><button onclick="window.inviteModal()" class="btn-primary w-auto px-6 py-3 text-xs">+ Invite User</button></div><div class="table-container"><div class="table-scroll"><table><tbody>
...
async function renderApprovals(c) {
    ...
    const rows = q.map(x => `<tr><td class="font-bold text-sm uppercase">${x.products?.name}</td><td class="font-bold text-blue-600">${x.quantity}</td><td class="text-xs text-gray-500 uppercase tracking-wide">To: ${x.to_loc?.name}</td><td class="text-right"><button onclick="window.approve('${x.id}','approved')" class="text-[10px] font-bold bg-slate-900 text-white px-3 py-2 rounded">APPROVE</button></td></tr>`).join('');
    c.innerHTML = `<h1 class="text-xl font-bold mb-8 uppercase text-gray-400">Approvals Center</h1><div class="table-container"><div class="table-scroll"><table><tbody>${rows.length?rows:'<tr><td colspan="4" class="text-center text-xs text-gray-400 py-8">No pending approvals.</td></tr>'}</tbody></table></div></div>`;
}
...
async function renderSettings(c) { 
    ...
    c.innerHTML = `<div class="flex justify-between items-center mb-10"><h1 class="text-2xl font-bold uppercase text-gray-900">Settings</h1><button onclick="window.addStoreModal()" class="btn-primary w-auto px-6 py-3 text-xs">+ Add Hub</button></div><div class="table-container"><div class="table-scroll"><table><tbody>${locs.map(l=>`<tr><td class="font-bold text-sm uppercase">${l.name}</td><td class="text-green-600 font-bold text-[9px] text-right uppercase tracking-wider">ACTIVE</td></tr>`).join('')}</tbody></table></div></div>`; 
}
...
window.issueModal = async (name, id, fromLoc) => { 
    ...
    document.getElementById('modal-content').innerHTML = `
        <h3 class="font-bold text-lg mb-8 uppercase tracking-tight text-center">Move Stock</h3>
        <div class="input-group"><label class="input-label">Product</label><input value="${name}" disabled class="input-field bg-gray-50 font-bold uppercase text-gray-500"></div>
