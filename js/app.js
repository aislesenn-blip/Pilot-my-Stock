...
window.onload = async () => {
    ...
    try {
        // 1. Jaribu kupata Profile
        profile = await getCurrentProfile(session.user.id);

        // 2. KAMA PROFILE HAIPO (Hapa ndipo palipokuwa panakwama mwanzo)
        if (!profile || !profile.organization_id) {
            console.log("Profile missing. Checking invites...");

            // Angalia kama ana invite (Manual Check)
            ...
            if (invite) {
                console.log("Invite Found! Creating Profile...");
                
                // TENGENEZA PROFILE KWA NGUVU (Javascript Insert)
                ...
                if (createError) {
                    ...
                    alert("System Error: Failed to initialize profile.");
                    logout();
                    return;
                }

                // Update Invite iwe 'accepted'
                ...
                // Kama hana invite kabisa, basi ni Manager mpya -> Nenda Setup
