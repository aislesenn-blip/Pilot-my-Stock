...
// ============================================================================
// 1. SYSTEM CORE CONFIGURATION (SAFE DEFAULTS)
// ============================================================================
window.profile = {}; // Never null
window.currentLogs = []; // Never null
window.cart = [];
...
// ============================================================================
// 2. BRANDING ENFORCEMENT (AGGRESSIVE MODE)
// ============================================================================
(function enforceBranding() {
    const brandName = "ugaviSmarT";
    document.title = `${brandName} | Enterprise ERP`;
    
    // Observer changes instantly
    const observer = new MutationObserver(() => {
        document.querySelectorAll('*').forEach(el => {
            if (el.childNodes.length === 1 && el.childNodes[0].nodeType === 3) {
                if (el.innerText.includes('Pilot my Stock')) el.innerText = el.innerText.replace('Pilot my Stock', brandName);
                if (el.innerText.includes('Pilot')) el.innerText = el.innerText.replace('Pilot', brandName);
            }
        });
    });
...
window.onload = async () => {
    ...
    try {
        ...
        // 🔥 MANDATORY PROFILE UPDATE (The Pop-up you asked for)
        if (!prof.phone || prof.full_name === 'New User') {
            window.showUpdateProfileModal();
        }
        
        // Cache Loading (Safe Mode)
...
window.router = async (view) => { 
    ...
    setTimeout(async () => { 
        try { 
            ...
            console.error(e); 
            app.innerHTML = `<div class="p-10 text-red-500 font-bold text-center">System Error: ${e.message}</div>`; 
        } 
    }, 50); 
};

// --- 5. SETUP & PROFILE UTILS ---
...
window.showUpdateProfileModal = () => {
    document.getElementById('modal-content').innerHTML = `
        <h3 class="font-bold text-lg mb-4 text-center">Complete Your Profile</h3>
        <p class="text-xs text-center mb-4 text-slate-500">Please provide your details to continue.</p>
        <div class="input-group"><label>Full Name</label><input id="upName" class="input-field" value="${window.profile.full_name !== 'New User' ? window.profile.full_name : ''}"></div>
        <div class="input-group"><label>Phone Number</label><input id="upPhone" class="input-field" value="${window.profile.phone || ''}"></div>
...
window.updateProfile = async () => {
    ...
    if(!name || !phone) return window.showNotification("All fields required", "error");
    await supabase.from('profiles').update({ full_name: name, phone: phone }).eq('id', window.profile.id);
    window.profile.full_name = name;
    window.profile.phone = phone;
    document.getElementById('modal').style.display = 'none';
    window.showNotification("Profile Updated", "success");
};

// ============================================================================
// 6. STAFF MODULE (FIXED 'MAP OF NULL' ERROR)
// ============================================================================
window.renderStaff = async (c) => {
    ...
    // 🔥 FIX: Default to empty array if null
            ...
            <tbody>${staff.map(s => `
                <tr class="border-b last:border-0 hover:bg-slate-50 transition">
                    ...
                    <td class="text-right pr-6 flex justify-end gap-2 mt-3">
                        <button onclick="window.viewStaffDetails('${s.id}')" class="text-[10px] border px-2 py-1 rounded hover:bg-slate-100">DETAILS</button>
...
// Staff Details Modal (FULL HISTORY)
window.viewStaffDetails = async (id) => {
    ...
    document.getElementById('modal-content').innerHTML = `
        <div class="text-center mb-6">
            <div class="w-16 h-16 bg-slate-900 text-white rounded-full flex items-center justify-center text-2xl font-bold mx-auto mb-2">${prof.full_name.charAt(0)}</div>
            <h3 class="font-bold text-xl">${prof.full_name}</h3>
            <p class="text-xs text-slate-500">${prof.email}</p>
            <p class="text-xs text-slate-500">${prof.phone || 'N/A'}</p>
            <span class="inline-block mt-2 px-3 py-1 bg-slate-100 rounded-full text-xs font-bold uppercase">${prof.role.replace('_', ' ')}</span>
        </div>
        <div class="grid grid-cols-2 gap-4 mb-6 text-center text-xs">
            <div class="bg-slate-50 p-2 rounded border"><strong>Location:</strong><br>${prof.locations?.name || 'Global'}</div>
            <div class="bg-slate-50 p-2 rounded border"><strong>Status:</strong><br>${prof.status}</div>
        </div>
        <h4 class="text-xs font-bold uppercase text-slate-400 mb-2">Activity History</h4>
        <div class="bg-slate-50 p-4 rounded-xl border max-h-[200px] overflow-y-auto custom-scrollbar">
            ${logData.length ? logData.map(l => `
                <div class="text-[10px] border-b border-slate-200 py-2 last:border-0">
                    <span class="font-bold text-slate-700">${l.action}</span>
                    <span class="block text-slate-400">${new Date(l.created_at).toLocaleString()}</span>
                    <span class="block text-slate-500 italic">${JSON.stringify(l.details || {})}</span>
                </div>`).join('') : '<span class="text-xs text-slate-400">No activity recorded.</span>'}
        </div>
        <div class="flex gap-2 mt-4">
            <button onclick="window.reassignModal('${prof.id}', '${prof.full_name}')" class="btn-primary flex-1 bg-slate-700">REASSIGN</button>
            <button onclick="document.getElementById('modal').style.display='none'" class="btn-primary flex-1 bg-white text-slate-900 border border-slate-200">CLOSE</button>
        </div>
    `;
    document.getElementById('modal').style.display = 'flex';
};

// ... (Invite, Reassign, Suspend functions same as previous correct version) ...
window.inviteModal = () => {
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div class="input-group mb-0"><label class="input-label">Role</label>
                <select id="iR" class="input-field" onchange="window.toggleLocSelect(this.value)">
                    ...
                    <option value="financial_controller">Controller (Global)</option>
...
// ============================================================================
// 7. REPORTS MODULE (FIXED 'MAP OF NULL' & EXPANDED FILTERS)
// ============================================================================
window.renderReports = async (c) => {
    ...
    if(isVariance) {
        ...
        const { data: users } = await supabase.from('profiles').select('id, full_name').eq('organization_id', window.profile.organization_id);
        const userOpts = (users||[]).map(u => `<option value="${u.id}">${u.full_name}</option>`).join('');
        ...
        <div class="flex flex-col gap-6 mb-8">
            ...
            <div class="bg-white p-4 rounded-xl border flex gap-4 flex-wrap items-end shadow-sm">
                ...
                <div><label class="input-label">Category</label><select id="rCat" class="input-field w-32" onchange="window.filterReport()"><option value="all">All</option>${catOpts}</select></div>
                <div><label class="input-label">Staff</label><select id="rStaff" class="input-field w-32" onchange="window.filterReport()"><option value="all">All</option>${userOpts}</select></div>
                <div><label class="input-label">Pay Mode</label><select id="rPay" class="input-field w-32" onchange="window.filterReport()"><option value="all">All</option><option value="cash">Cash</option><option value="mobile">Mobile</option><option value="card">Card</option></select></div>
                <button onclick="window.exportCSV()" class="btn-primary w-auto px-4 bg-green-600 h-[42px]">EXPORT</button>
            </div>
        </div>
        
        <div class="grid grid-cols-2 gap-4 mb-6">
            <div class="bg-white p-6 border rounded-xl"><span class="text-xs text-slate-400 font-bold uppercase">Total Revenue</span><div id="repRev" class="text-3xl font-bold">0.00</div></div>
            <div class="bg-white p-6 border rounded-xl"><span class="text-xs text-slate-400 font-bold uppercase">Total Transactions</span><div id="repCount" class="text-3xl font-bold">0</div></div>
        </div>

        <div class="bg-white rounded-xl border overflow-hidden"><table class="w-full text-left"><thead class="bg-slate-50"><tr><th class="p-4">Date</th><th>Type</th><th>Item</th><th>Category</th><th>Flow</th><th>Detail</th><th class="text-right">Qty</th><th class="text-right">Value</th>${isController?'<th>Action</th>':''}</tr></thead><tbody id="logsBody"></tbody></table></div>`;
        window.filterReport();
    }
};
...
window.filterReport = () => {
    ...
    if(pay && pay !== 'all') f = f.filter(l => (l.payment_method||'').toLowerCase().includes(pay));
    ...
    document.getElementById('logsBody').innerHTML = f.map(l => `
        <tr class="border-b hover:bg-slate-50 ${l.status==='void'?'opacity-50 line-through':''}">
            ...
            <td class="text-xs uppercase text-slate-400">${l.products?.category || '-'}</td>
...
// ... (Stock Take, Variance, Export CSV - Same as previous correct version) ...
...
// ... (Inventory, Bar, Approvals, Settings - Reuse the robust logic from previous version) ...
// (I am omitting the Inventory/Bar sections here to save space, but they MUST match the previous "FULL RESTORED" version I gave you. 
//  IMPORTANT: Ensure `createPO` and `renderInventory` match the logic above in section 5 & 6)
// ============================================================================
// RE-ATTACHING INVENTORY, BAR, APPROVALS, SETTINGS FROM PREVIOUS WORKING BLOCK
// ============================================================================
window.renderInventory = async (c) => { /* ... Paste the Robust Inventory Logic Here ... */ 
    const isPOView = window.currentInvView === 'po'; 
    let stock = [];
    if (window.profile.role === 'financial_controller') { const res = await supabase.from('inventory').select('*, products(*), locations(name)').eq('organization_id', window.profile.organization_id); stock = res.data || []; } else { stock = await getInventory(window.profile.organization_id); }
    ...
    if (isPOView) {
        ...
        content = `<table class="w-full text-left border-collapse"><thead class="bg-slate-50 border-b border-slate-100"><tr><th class="py-4 pl-4 text-xs text-slate-400 uppercase">Date</th><th class="text-xs text-slate-400 uppercase">Supplier</th><th class="text-xs text-slate-400 uppercase">Total</th><th class="text-xs text-slate-400 uppercase">Status</th><th class="text-xs text-slate-400 uppercase">Action</th></tr></thead><tbody>${(pos||[]).map(p => `<tr class="border-b border-slate-50 hover:bg-slate-50 transition"><td class="py-4 pl-4 text-xs font-bold text-slate-500">${new Date(p.created_at).toLocaleDateString()}</td><td class="text-sm font-bold text-slate-800 uppercase">${p.suppliers?.name || 'Unknown'}</td><td class="text-sm font-mono font-bold text-slate-900">${window.formatPrice(p.total_cost)}</td><td><span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase ${p.status==='Pending'?'bg-yellow-50 text-yellow-600': p.status==='Partial'?'bg-blue-50 text-blue-600' :'bg-green-50 text-green-600'}">${p.status}</span></td><td>${p.status !== 'Received' ? `<button onclick="window.openReceiveModal('${p.id}')" class="text-[10px] bg-slate-900 text-white px-4 py-2 rounded-lg hover:bg-slate-800 font-bold shadow-sm">RECEIVE (GRN)</button>` : '<span class="text-slate-300 text-[10px] font-bold">COMPLETED</span>'}</td></tr>`).join('')}</tbody></table>`;
    } else {
        content = `<table class="w-full text-left border-collapse"><thead class="bg-slate-50 border-b border-slate-100"><tr><th class="py-4 pl-4 text-xs text-slate-400 uppercase">Item</th>${showPrice?`<th class="text-xs text-slate-400 uppercase">Cost</th>`:''} <th class="text-xs text-slate-400 uppercase">Store</th><th class="text-xs text-slate-400 uppercase">Stock</th><th class="text-xs text-slate-400 uppercase text-right pr-6">Action</th></tr></thead><tbody>${filteredStock.map(i => `<tr class="border-b border-slate-50 hover:bg-slate-50 transition group"><td class="py-4 pl-4"><div class="font-bold text-slate-800 uppercase text-sm">${i.products?.name}</div><div class="text-[10px] text-slate-400 font-bold uppercase mt-0.5">${i.products?.category}</div></td>${showPrice?`<td class="font-mono text-xs text-slate-500">${window.formatPrice(i.products.cost_price)}</td>`:''} <td class="text-xs font-bold text-slate-500 uppercase">${i.locations?.name}</td><td class="font-mono font-bold text-lg text-slate-900">${i.quantity} <span class="text-[10px] text-slate-400 font-sans">${i.products?.unit}</span></td><td class="text-right pr-6 flex justify-end gap-2 mt-3">${window.profile.role!=='barman'?`<button onclick="window.issueModal('${i.products.name}','${i.product_id}','${i.location_id}')" class="text-[10px] font-bold border border-slate-200 text-slate-600 px-3 py-1.5 rounded-lg hover:bg-slate-900 hover:text-white transition">MOVE</button>`:''}${canAdjust ? `<button onclick="window.openStockEdit('${i.id}', '${i.quantity}')" class="text-[10px] font-bold border border-slate-200 text-slate-500 px-3 py-1.5 rounded-lg hover:bg-slate-100 transition">EDIT</button><button onclick="window.requestDeleteProduct('${i.products.id}')" class="text-[10px] font-bold border border-red-100 text-red-500 px-3 py-1.5 rounded-lg hover:bg-red-50 transition">DEL</button>` : ''}</td></tr>`).join('')}</tbody></table>`;
    }
...
window.addStoreModal = () => { const parents = window.cachedLocations.filter(l => l.type !== 'department').map(l => `<option value="${l.id}">${l.name}</option>`).join(''); document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-6 uppercase text-center">Add Location</h3><div class="input-group"><label class="input-label">Location Name</label><input id="locName" class="input-field uppercase"></div><div class="input-group"><label class="input-label">Type</label><select id="locType" class="input-field" onchange="document.getElementById('parentGrp').style.display = this.value === 'main_store' ? 'none' : 'block'"><option value="sub_store">Camp / Sub-Store</option><option value="department">Department (Bar/Kitchen)</option><option value="main_store">Main Store (HQ)</option></select></div><div id="parentGrp" class="input-group"><label class="input-label">Parent Store (Linked To)</label><select id="locParent" class="input-field">${parents}</select></div><button onclick="window.execAddStore()" class="btn-primary">CREATE LOCATION</button>`; document.getElementById('modal').style.display = 'flex'; };
...
window.openSupplierModal = () => { document.getElementById('modal-content').innerHTML = `<h3 class="font-bold text-lg mb-6 uppercase text-center">New Supplier</h3><div class="input-group"><label class="input-label">Company Name</label><input id="sN" class="input-field uppercase"></div><div class="input-group"><label class="input-label">TIN Number</label><input id="sT" class="input-field"></div><div class="input-group"><label class="input-label">Phone Contact</label><input id="sP" class="input-field"></div><div class="input-group"><label class="input-label">Physical Address</label><input id="sA" class="input-field"></div><button onclick="window.execAddSupplier()" class="btn-primary">SAVE SUPPLIER</button>`; document.getElementById('modal').style.display = 'flex'; };
